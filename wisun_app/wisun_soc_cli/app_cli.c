...
static const app_enum_t app_frame_type_enum[] = {
  ...
  { "lxpm_state", SL_WISUN_FRAME_TYPE_LXPM_STATE },
...
static void app_handle_lxpm_neighbor_updated_ind(sl_wisun_evt_t *evt)
{
  char ipv6_string[40];
  ip6tos(&evt->evt.lxpm_neighbor_updated.link_local_ipv6.address, ipv6_string);
  printf("[LXPM neighbor updated: %s", ipv6_string);
  printf(" with %"PRIu16" group(s):\r\n", evt->evt.lxpm_neighbor_updated.group_count);
  for (uint16_t i = 0; i < evt->evt.lxpm_neighbor_updated.group_count; i++) {
    ip6tos(evt->evt.lxpm_neighbor_updated.groups[i].address, ipv6_string);
    printf("    %s\r\n", ipv6_string);
  }
  printf("]\r\n");
}
...
void sl_wisun_on_event(sl_wisun_evt_t *evt)
{
  ...
  switch (evt->header.id) {
    ...
    case SL_WISUN_MSG_FB_READY_IND_ID:
      ...
      break;
    case SL_WISUN_MSG_LXPM_NEIGHBOR_UPDATED_IND_ID:
      app_handle_lxpm_neighbor_updated_ind(evt);
      break;
    default:
...
static sl_status_t app_set_options(void)
{
  struct {
    ...
    { SL_WISUN_OPTION_TRAFFIC_ALLOW_EDFE_ALL_PACKETS,  &app_settings_wisun.allow_edfe_all_packets,  sizeof(app_settings_wisun.allow_edfe_all_packets) },
...
void app_clear_event_log_filter(sl_cli_command_arg_t *arguments)
{
  sl_status_t ret;
  (void)arguments;
...
static void update_app_settings(sl_wisun_option_id_t option_id, uint32_t option_value)
{
  ...
  switch (option_id) {
    ...
    case SL_WISUN_OPTION_TRAFFIC_MAX_EDFE_FRAGMENT_COUNT:
      ...
      break;
    case SL_WISUN_OPTION_TRAFFIC_ALLOW_EDFE_ALL_PACKETS:
      app_settings_wisun.allow_edfe_all_packets = (uint8_t)option_value;
      break;
...
#if defined(SL_CATALOG_WISUN_LXPM_PRESENT)
