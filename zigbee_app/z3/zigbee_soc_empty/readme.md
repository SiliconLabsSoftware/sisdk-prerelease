...
This is a board-independent Zigbee SoC empty template. It includes only the essential Zigbee stack and application-framework components needed to join a Zigbee 3.0 network. The project has no buttons, LEDs, display, CLI, or other WSTK-specific peripherals, so it can be created from a device OPN (not only from a development board).
...
**`SL_ZIGBEE_EMPTY_JOIN_ON_BOOT`** in `config/zigbee_soc_empty_config.h` (default **FALSE**) controls testing behavior: when enabled, the app leaves any restored NVM network on boot and steers again. To move a device to a different network, mass erase and reflash (or enable this option for lab use).

This application does **not** include a CLI. To verify join, use a second device with CLI (for example **Zigbee - SoC Z3 Light**). Form or open a network on that device, then reset Empty. As a **router**, check Light **neighbor-table**; as an **end device**, check **child-table**.
...
```bash
slc generate \
  -p zigbee_app/z3/zigbee_soc_empty/zigbee_soc_empty.slcp \
  --with EFR32MG24B220F1536IM48,toolchain_gcc \
  -d /tmp/zigbee_soc_empty_router

cd /tmp/zigbee_soc_empty_router && make -j -f zigbee_soc_empty.Makefile
```

Use any Zigbee-capable device OPN in place of `EFR32MG24B220F1536IM48`. No board is required because this template has no UART/VCOM dependency.
...
slc generate \
  -p zigbee_app/z3/zigbee_soc_empty/zigbee_soc_empty.slcp \
  --with EFR32MG24B220F1536IM48,toolchain_gcc,zigbee_pro_leaf_stack,zigbee_end_device_support \
  ...
  -d /tmp/zigbee_soc_empty_end_device

cd /tmp/zigbee_soc_empty_end_device && make -j -f zigbee_soc_empty.Makefile
```
...
Then **flash or mass erase Empty** (first join) or enable **`SL_ZIGBEE_EMPTY_JOIN_ON_BOOT`** in `config/zigbee_soc_empty_config.h` and reset Empty to rejoin after a previous join.
