...
typedef struct RMR_State{
  ...
  sl_rail_frame_type_t frameTypeConfig;
  uint16_t frameLenList[RMR_FRAME_LENGTH_LIST_LEN];
  uint32_t frameCodingTable[RMR_FRAME_CODING_TABLE_LEN];
  sl_rail_channel_config_entry_attr_t generatedEntryAttr;
  sl_rail_channel_config_entry_t generatedChannels[1];
  __ALIGNED(4) uint8_t convDecodeBuffer[RMR_CONV_DECODE_BUFFER_LEN];
  sl_rail_channel_config_t channelConfig;
  uint8_t dcdcRetimingConfig[RMR_DCDC_RETIMING_LEN];
  __ALIGNED(4) uint8_t hfxoRetimingConfig[RMR_HFXO_RETIMING_LEN];
  uint32_t rffpllConfig[RMR_RFFPLL_CONFIG_LEN];
  __ALIGNED(4) uint8_t txIrCalConfig[RMR_TXIRCAL_CONFIG_LEN];
  uint32_t timingConfig[RMR_TIMING_CONFIG_LEN];
} RMR_State_t;
...
sl_rail_status_t Rmr_updateConfigurationPointer(uint8_t structToModify, uint16_t offset, uint8_t structToPointTo);
sl_rail_status_t Rmr_reconfigureModem(sl_rail_handle_t railHandle);
...
sl_rail_status_t Rmr_updateConfigurationPointer(uint8_t structToModify, uint16_t offset, uint8_t structToPointTo)
{
  ...
  switch (structToModify) {
    ...
    case (RMR_STRUCT_FRAME_TYPE_CONFIG): {
      ...
      rmrState->frameTypeConfig.p_frame_byte_lengths = (uint16_t *)structPointer;
      break;
    }
    default: {
      // Error unrecognized structure
...
sl_rail_status_t Rmr_reconfigureModem(sl_rail_handle_t railHandle)
{
  ...
  sl_rail_include_frame_type_length(railHandle);
  ...
  sl_rail_idle(railHandle, SL_RAIL_IDLE_FORCE_SHUTDOWN_CLEAR_FLAGS, false);
...
static bool rmrInit(sl_cli_command_arg_t *args)
{
  ...
  #else
    rmrState->channelConfig.phy_config_delta_subtract = NULL;
  #endif
    rmrState->channelConfig.p_entries = rmrState->generatedChannels;
    rmrState->channelConfig.number_of_entries = 1;
    rmrState->channelConfig.signature = 0U;
    rmrState->generatedChannels[0].phy_config_delta_add = NULL;
    rmrState->generatedChannels[0].p_attr = &rmrState->generatedEntryAttr;
  }
  return true;
}
...
void CI_printRmrStructureLocations(sl_cli_command_arg_t *args)
{
  ...
  responsePrintMulti("Id:%u,Address:0x%x,Size:%u",
                     RMR_STRUCT_TIMING_CONFIG,
                     (uint8_t *) &(rmrState->timingConfig),
                     sizeof(rmrState->timingConfig));
}
...
void CI_writeRmrStructure(sl_cli_command_arg_t *args)
{
  ...
  RMR_StructureIndex_t structure = sl_cli_get_argument_uint8(args, RMR_CI_RMR_STRUCTURE);
  ...
  if (Rmr_writeRmrStructure(structure, offset, count, bufferedData) != SL_RAIL_STATUS_NO_ERROR) {
...
void CI_updateConfigurationPointer(sl_cli_command_arg_t *args)
{
  ...
  if (Rmr_updateConfigurationPointer(structure, location, pointer) != SL_RAIL_STATUS_NO_ERROR) {
...
void CI_reconfigureModem(sl_cli_command_arg_t *args)
{
  ...
  if (Rmr_reconfigureModem(railHandle) == SL_RAIL_STATUS_NO_ERROR) {
    responsePrint(sl_cli_get_command_string(args, 0), "CommandStatus:Success");
  } else {
