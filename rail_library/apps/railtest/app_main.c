...
// Function called from app_process_action within the main super loop.
...
static void RAILCb_RssiAverageDone(RAIL_Handle_t railHandle)
{
  RailAppEvent_t *rssi = (RailAppEvent_t *)sl_malloc(sizeof(RailAppEvent_t));
  if (rssi == NULL) {
    eventsMissed++;
    return;
  }
...
void sl_rail_util_on_event(RAIL_Handle_t railHandle, RAIL_Events_t events)
{
  ...
  if (events & RAIL_EVENT_TX_CCA_ACTIVATED) {
    counters.lbtCcaActivated++;
  }
  if (events & RAIL_EVENT_TX_CCA_RETRY) {
    if (RAIL_IsNextCcaNow(railHandle)) {
...
void enqueueEvents(RAIL_Events_t events)
{
  ...
  if (events != RAIL_EVENTS_NONE) {
    ...
    RAIL_Time_t eventTime = RAIL_GetTime();
