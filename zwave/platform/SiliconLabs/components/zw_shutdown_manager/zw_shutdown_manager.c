...
#ifdef SL_CATALOG_ZW_APP_TIMER_DEEP_SLEEP_PRESENT
    if (AppTimerDeepSleepPersistentIsAnyTimerExpiringSoon()) {
      ZPAL_LOG_DEBUG(ZPAL_LOG_SHUTDOWN_MANAGER, "Keeping the device in EM2 because DeepSleepPersistentTimer is about to fire\n");
      sl_dcdc_setup_em2();
      return;
    }
#endif
