...
ZW_ADD_CMD(FUNC_ID_PROP_JAMMING_DETECTION_COMMAND)
{
  ...
  switch (subcmd) {
    ...
    case FUNC_ID_PROP_JAMMING_SUBCOMMAND_CONFIGURATION:
      ...
      if (4 == frame_payload_len(frame)) {
        ...
        /* Success response: 4 bytes (subcmd, channel, threshold, trigger). */
        if (ZPAL_STATUS_OK == status) {
          compl_workbuf[0] = FUNC_ID_PROP_JAMMING_SUBCOMMAND_CONFIGURATION;
