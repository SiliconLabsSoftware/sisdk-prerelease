...
#include "bin/test_gblv4_encrypted.c"
#include "bin/test_gblv4_plain.c"
...
static void testGblv4ParseEncImagePass(void)
{
  ...
  while (offset < (sizeof(gblv4_encrypted) / sizeof(gblv4_encrypted[0]))) {
    if (offset < ((sizeof(gblv4_encrypted) - 64) / sizeof(gblv4_encrypted[0]))) {
      retval = bootloader_parseBuffer(parseCtx,
                                      &callbacks,
                                      (uint8_t*)&(gblv4_encrypted[offset]),
                                      64);
      offset += 64 / sizeof(gblv4_encrypted[0]);
    } else {
      retval = bootloader_parseBuffer(parseCtx,
                                      &callbacks,
                                      (uint8_t*)&(gblv4_encrypted[offset]),
                                      4);
      offset += 4 / sizeof(gblv4_encrypted[0]);
    }
  ...
  TEST_ASSERT_EQUAL(BOOTLOADER_OK,
                    bootloader_parseImageInfo(
                      parseCtx,
                      (uint8_t*)&gblv4_encrypted,
                      sizeof(gblv4_encrypted),
                      &appInfo,
                      &blVersion));

  printAppData(&appInfo);
  TEST_ASSERT_EQUAL_HEX32(0x0A0B000C, blVersion);
...
static void testGblv4ParsePlainImageFail(void)
{
  TEST_ASSERT_EQUAL(BOOTLOADER_OK, bootloader_init());
  ...
  while (offset < (sizeof(gblv4_plain) / sizeof(gblv4_plain[0]))) {
    if (offset < ((sizeof(gblv4_plain) - 64) / sizeof(gblv4_plain[0]))) {
      retval = bootloader_parseBuffer(parseCtx,
                                      &callbacks,
                                      (uint8_t*)&(gblv4_plain[offset]),
                                      64);
      offset += 64 / sizeof(gblv4_plain[0]);
    } else {
      retval = bootloader_parseBuffer(parseCtx,
                                      &callbacks,
                                      (uint8_t*)&(gblv4_plain[offset]),
                                      4);
      offset += 4 / sizeof(gblv4_plain[0]);
    }

    if (retval == BOOTLOADER_ERROR_PARSER_FILETYPE) {
      break;
    }
  ...
  TEST_ASSERT_TRUE(retval == BOOTLOADER_ERROR_PARSER_FILETYPE);
  TEST_ASSERT_EQUAL(BOOTLOADER_OK, bootloader_deinit());
}
...
int main(void)
{
  ...
  UnityBegin("GBLv4_ENC_INTERFACE");
  TEST_PRINTF("App version %d\n", sl_app_properties.app.version);

  RUN_TEST(testGblv4ParseEncImagePass, __LINE__);
  RUN_TEST(testGblv4ParsePlainImageFail, __LINE__);
