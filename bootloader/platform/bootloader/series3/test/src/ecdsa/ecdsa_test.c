...
void test_nist_fips_186_4_vectors(void)
{
  ...
  for (size_t i = 0; i < sizeof(nist_fips_186_4) / sizeof(nist_fips_186_4[0]); i++) {
    ...
    btl_initSha256(&sha_ctx, &cmd_ctx);
    btl_updateSha256(&sha_ctx, &cmd_ctx, msg, 128);
    btl_finalizeSha256(&sha_ctx, &cmd_ctx);
    btl_initSha256(&sha_ctx_two, &cmd_ctx);
    btl_updateSha256(&sha_ctx_two, &cmd_ctx, msg, 128);
    btl_finalizeSha256(&sha_ctx_two, &cmd_ctx);
    ...
    TEST_PRINTF("FIPS-186-4 vector %u, expect %c\n", i, expect_pass);
    if (expect_pass == 'P') {
      TEST_ASSERT_EQUAL(BOOTLOADER_OK,
