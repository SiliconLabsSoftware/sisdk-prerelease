...
static bool spiflashVerifyAddressRange(uint32_t                address,
                                       uint32_t                length)
{
...
static bool spiflashVerifyErased(uint32_t address, uint32_t len)
{
  waitUntilNotBusy();
...
int32_t storage_readRaw(uint32_t address, uint8_t *data, size_t length)
{
  ...
  if (!spiflashVerifyAddressRange(address, length)) {
...
int32_t storage_writeRaw(uint32_t address, uint8_t *data, size_t numBytes)
{
  ...
  // Ensure address is is within chip
  if (!spiflashVerifyAddressRange(address, numBytes)) {
    return BOOTLOADER_ERROR_STORAGE_INVALID_ADDRESS;
  }
  // Ensure space is empty
  if (!spiflashVerifyErased(address, numBytes)) {
...
int32_t storage_eraseRaw(uint32_t address, size_t totalLength)
{
  ...
  if (!spiflashVerifyAddressRange(address, totalLength)) {
    return BOOTLOADER_ERROR_STORAGE_INVALID_ADDRESS;
  }
