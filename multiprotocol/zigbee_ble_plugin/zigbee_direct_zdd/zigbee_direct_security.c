...
#define ZIGBEE_DIRECT_KEY_CALC_INPUT_SIZE (EUI64_SIZE + SL_ZIGBEE_ENCRYPTION_KEY_SIZE)
#define ZIGBEE_DIRECT_BASIC_KEY_DERIVATION_BYTE 0x03
...
void sli_zigbee_direct_anonymous_join_event_handler(sl_zigbee_af_event_t *event)
{
  ...
  if ((sl_zigbee_direct_anonymous_join_timeout_sec > 0) && (sl_zigbee_direct_anonymous_join_timeout_sec != ZIGBEE_DIRECT_ANONYMOUS_JOIN_TIMEOUT_INVALID)) {
...
sl_status_t sli_zigbee_direct_calculate_basic_key(sl_802154_long_addr_t zvd_IEEE, uint8_t* basic_key)
{
  ...
  uint8_t input[ZIGBEE_DIRECT_KEY_CALC_INPUT_SIZE];
  uint8_t data = ZIGBEE_DIRECT_BASIC_KEY_DERIVATION_BYTE;
  uint8_t i;
  ...
  for (i = 0; i < SL_ZIGBEE_ENCRYPTION_KEY_SIZE; i++) {
    sl_zigbee_app_debug_print(" %02X", input[i + EUI64_SIZE]);
  }
  sl_zigbee_app_debug_println("]");

  if (SL_STATUS_OK != sl_zigbee_aes_hash_simple(ZIGBEE_DIRECT_KEY_CALC_INPUT_SIZE, input, (uint8_t*)&hash.key)) {
    return SL_STATUS_FAIL;
  }
...
sl_status_t sli_zigbee_direct_calculate_admin_key(sl_802154_long_addr_t zvd_IEEE, uint8_t* admin_key)
{
  ...
  sl_zigbee_sec_man_key_t network_key;
  uint8_t input[ZIGBEE_DIRECT_KEY_CALC_INPUT_SIZE];
  uint8_t data = ZIGBEE_DIRECT_ADMIN_KEY_DERIVATION_BYTE;
  ...
  if (SL_STATUS_OK != sl_zigbee_aes_hash_simple(ZIGBEE_DIRECT_KEY_CALC_INPUT_SIZE, input, (uint8_t*) &hash)) {
    return SL_STATUS_FAIL;
  }
  context.core_key_type = SL_ZB_SEC_MAN_KEY_TYPE_INTERNAL;
  sl_zigbee_sec_man_import_key(&context, &hash);
...
{
  const uint8_t uuid[UUID_SIZE] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x00, 0x80, 0x00, 0x00, 0x80, 0x5F, 0x9B, 0x34, 0xFB };
...
static bool sli_zigbee_direct_extract_value_from_previous_uuid(uint16_t handle, uint8_t *uuid, size_t *size, uint8_t *data)
{
  ...
  while (handle_pointer) {
    ...
    if ((status == SL_STATUS_OK) && (c_location < handle)) {
...
sl_status_t sli_zigbee_direct_security_encrypt_packet(sl_802154_long_addr_t sourceEui, uint8_t *encryptData, uint16_t dataLen, uint16_t handle)
{
  ...
  authData[UUID_SIZE] = 0x00; // Service Instance (SHALL be 0)
...
static int sli_zigbee_direct_calculate_mac_tag_value(bool useKC_2_U, uint8_t *output, uint8_t method)
{
  ...
  if (method != DLK_ECC_P256_SHA_256) {
    ...
    if (ret == 0) {
      ...
      if (useKC_2_U) {
        memcpy(input, "KC_2_U", 6);
      } else {
        memcpy(input, "KC_2_V", 6);
      }

      memcpy(&input[6], sl_zigbee_get_eui64(), EUI64_SIZE);
      memcpy(&input[14], &ourPointTlv.value[EUI64_SIZE], DLK_ECC_COORDINATE_SIZE);
      memcpy(&input[46], sl_zvd_eui, EUI64_SIZE);
  ...
  switch (writeValue->data[0]) {
    case SESSION_ESTABLISHMENT_MSG_1:
      ...
      if ( sl_zigbee_tlv_search_buffer_payload_for_id(buffer_len, buffer_ptr, 0, SL_ZIGBEE_DIRECT_SECURITY_TLV_P256_PUBLIC_POINT_TAG_ID, (sl_zigbee_tlv_t *) &sl_tlv_pointer1, 78 /*writeValue->len - 1*/) == SL_STATUS_OK) {
        ...
        for (i = 0; i < DLK_ECC_P256_PUBLIC_KEY_SIZE; i++) {
          sl_zigbee_app_debug_print(" %02X", sl_tlv_pointer1.value[i + EUI64_SIZE]);
        }
        sl_zigbee_app_debug_println("]");
        memcpy(sl_zvd_eui, &sl_tlv_pointer1.value[0], EUI64_SIZE);
        memcpy(sl_zvd_public_point_x, &sl_tlv_pointer1.value[EUI64_SIZE], DLK_ECC_COORDINATE_SIZE);
        memcpy(sl_zvd_public_point_y, &sl_tlv_pointer1.value[EUI64_SIZE + DLK_ECC_COORDINATE_SIZE], DLK_ECC_COORDINATE_SIZE);
        ...
        for (i = 0; i < DLK_ECC_COORDINATE_SIZE; i++) {
          sl_zigbee_app_debug_print(" %02X", sl_tlv_pointer1.value[i + EUI64_SIZE]);
        }
        sl_zigbee_app_debug_println("]");
        memcpy(sl_zvd_eui, &sl_tlv_pointer1.value[0], EUI64_SIZE);
    ...
    case SESSION_ESTABLISHMENT_MSG_3:
