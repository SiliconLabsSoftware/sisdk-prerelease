                                                                  ...
                                                                  uint8_t size_d)
{
  (void)data;
  UNUSED_VAR(size_d);
  if (((packetType == SL_ZIGBEE_ZIGBEE_PACKET_TYPE_NWK_DATA) || (packetType == SL_ZIGBEE_ZIGBEE_PACKET_TYPE_NWK_COMMAND))
      && (sl_zvd_node_id < SL_ZIGBEE_BROADCAST_ADDRESS) // Don't check if ZVD node ID is still set to SL_ZIGBEE_BROADCAST_ADDRESS
      && (sl_util_fetch_low_high_int16u(&packetData[2]) == sl_zvd_node_id)) {
    #ifdef SL_CATALOG_ZIGBEE_DEBUG_PRINT_PRESENT
