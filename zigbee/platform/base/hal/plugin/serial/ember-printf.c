...
#define flushBuffer()                                                \
  do { count = localBufferPointer - localBuffer;                     \
       if (flushHandler(port, localBuffer, count) != SL_STATUS_OK) { \
         goto fail; }                                                \
       total += count;                                               \
    ...
    if (next != '%' && !(stillInsideFormatSpecifier)) {
      ...
      switch (*string) {
        ...
        case 's': {
          ...
          ;
          if (flushHandler(port, arg, len) != SL_STATUS_OK) {
            goto fail;
          }
          total += len;
          break;
                                         ...
                                         uint16_t count)
{
  sl_status_t status;
  uint16_t i;
  status = sli_legacy_serial_write_string(port, prefix);
  if (status != SL_STATUS_OK) {
    return status;
  }
  for (i = 0; i < count; i++) {
    status = sli_legacy_serial_printf(port,
                                      "%x%s",
                                      bytes[i],
                                      (i < count - 1
                                       ? " "
...
sl_status_t sl_legacy_serial_print_bytes_line(uint8_t port,
                                              ...
                                              uint16_t count)
{
  sl_status_t result = sl_legacy_serial_print_bytes(port, prefix, bytes, count);

  if (result != SL_STATUS_OK) {
    return result;
  }

  return sli_legacy_serial_printf_line(port, "");
}
...
#ifndef SL_ZIGBEE_MANAGEMENT_SERIAL
  #if CORTEXM3_EFM32_MICRO

sl_status_t sli_legacy_serial_write_hex(uint8_t port, uint8_t dataByte)
{
  switch (COM_WriteHex((COM_Port_t) port, dataByte)) {
...
{
  Ecode_t com_status = COM_PrintfVarArg((COM_Port_t) port, formatString, ap);
  return ((com_status == ECODE_OK) ? SL_STATUS_OK : SL_STATUS_FAIL);
}
...
sl_status_t sli_legacy_serial_printf_var_arg(uint8_t port, const char *formatString, va_list ap)
{
  ...
  #ifdef SL_LEGACY_SERIAL_USE_STDIO
  if (!sli_util_printf_internal(sli_legacy_serial_write_data, port, formatString, ap)) {
    stat = SL_STATUS_FAIL;
  }
...
#endif //SL_ZIGBEE_MANAGEMENT_SERIAL
