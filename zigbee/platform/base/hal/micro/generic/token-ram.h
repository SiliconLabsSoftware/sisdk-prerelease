/***************************************************************************//**
 * @file
 * @brief Ram based token system for storing "non-volatile" information.
 *
 * In the simulator or during development, if a full hardware based token
 * system is not available or needs to be faked, the ram based token system
 * can be used.  This system mimicks the standard EEPROM model by declaring an
 * array in memory and accessing the array as if it was EEPROM.
 *
 * \b Note:  Don't forget that this is volatile.  The tokens are set to their
 * default value set upon bootup, and any specific token information must be
 * explicitly set (such as the EUI64).
 *******************************************************************************
 * # License
...
// TODO: Correctly handle manufacturing tokens for simulation.
// Manufacturing tokens should be initialized to FF and should only be
// writeable once at runtime.
...
#define halCommonWriteRtccRam(index, value, len) \
  halInternalWriteRtccRam(index, value, len)

#define halCommonReadRtccRam(inex, data, len) \
  halInternalReadRtccRam(index, data, len)
