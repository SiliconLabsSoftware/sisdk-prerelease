 ...
 *
 * @return A ::sl_status_t value indicating the success or failure of the
 * command:
 *  - SL_STATUS_FAIL if the \c eraseType is not valid
 *  - SL_STATUS_FLASH_ERASE_FAILED if erasing failed due to write protection
 *  - SL_STATUS_FLASH_VERIFY_FAILED if erase verification failed
 *  - SL_STATUS_OK if erasure completed and verified properly
 */
 ...
 * @return A ::sl_status_t value indicating the success or failure of the
 * command:
 *  - SL_STATUS_FLASH_PROGRAM_FAILED if the address is not half-word aligned, the
 ...
 *  - SL_STATUS_FLASH_VERIFY_FAILED if write verification failed
 *  - SL_STATUS_OK if writing completed and verified properly
 */
 ...
 * command:
 *  - SL_ZIGBEE_ERR_FLASH_PROG_FAIL if the byte chosen is greater than 7, write
 *    protection is enabled, or the byte is being written to more than twice
 *    between erasures.
 *  - SL_ZIGBEE_ERR_FLASH_VERIFY_FAILED if write verification failed
 *  - SL_ZIGBEE_SUCCESS if writing completed and verified properly
 */
