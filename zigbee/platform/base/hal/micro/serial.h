 ...
 *
 * Some functions in this file return an ::EmberStatus value and others return
 * ::sl_status_t. See error-def.h for definitions of all ::EmberStatus return
 * values, and sl_status.h for all ::sl_status_t return values.
 *
 ...
 * can test for.
 */
#define SL_LEGACY_SERIAL_UNUSED 0
#define SL_LEGACY_SERIAL_FIFO   1
#define SL_LEGACY_SERIAL_LOWLEVEL 2
...
#ifdef SL_LEGACY_SERIAL0_MODE
#if (SL_LEGACY_SERIAL0_MODE != SL_LEGACY_SERIAL_FIFO)      \
  && (SL_LEGACY_SERIAL0_MODE != SL_LEGACY_SERIAL_LOWLEVEL) \
  && (SL_LEGACY_SERIAL0_MODE != SL_LEGACY_SERIAL_UNUSED)
  #error Invalid Serial 0 Mode
#endif
#else
  #define SL_LEGACY_SERIAL0_MODE SL_LEGACY_SERIAL_UNUSED
#endif
#ifdef SL_LEGACY_SERIAL1_MODE
#if (SL_LEGACY_SERIAL1_MODE != SL_LEGACY_SERIAL_FIFO)      \
  && (SL_LEGACY_SERIAL1_MODE != SL_LEGACY_SERIAL_LOWLEVEL) \
  && (SL_LEGACY_SERIAL1_MODE != SL_LEGACY_SERIAL_UNUSED)
  #error Invalid Serial 1 Mode
#endif
#else
  #define SL_LEGACY_SERIAL1_MODE SL_LEGACY_SERIAL_UNUSED
#endif
#ifdef SL_LEGACY_SERIAL2_MODE
#if (SL_LEGACY_SERIAL2_MODE != SL_LEGACY_SERIAL_FIFO)      \
  && (SL_LEGACY_SERIAL2_MODE != SL_LEGACY_SERIAL_LOWLEVEL) \
  && (SL_LEGACY_SERIAL2_MODE != SL_LEGACY_SERIAL_UNUSED)
  #error Invalid Serial 2 Mode
#endif
#else
  #define SL_LEGACY_SERIAL2_MODE SL_LEGACY_SERIAL_UNUSED
#endif
#ifdef SL_LEGACY_SERIAL3_MODE
#if (SL_LEGACY_SERIAL3_MODE != SL_LEGACY_SERIAL_FIFO)      \
  && (SL_LEGACY_SERIAL3_MODE != SL_LEGACY_SERIAL_LOWLEVEL) \
  && (SL_LEGACY_SERIAL3_MODE != SL_LEGACY_SERIAL_UNUSED)
  #error Invalid Serial 3 Mode
#endif
#else
  #define SL_LEGACY_SERIAL3_MODE SL_LEGACY_SERIAL_UNUSED
#endif
#ifdef SL_LEGACY_SERIAL4_MODE
#if (SL_LEGACY_SERIAL4_MODE != SL_LEGACY_SERIAL_FIFO)      \
  && (SL_LEGACY_SERIAL4_MODE != SL_LEGACY_SERIAL_LOWLEVEL) \
  && (SL_LEGACY_SERIAL4_MODE != SL_LEGACY_SERIAL_UNUSED)
  #error Invalid Serial 4 Mode
#endif
#else
  #define SL_LEGACY_SERIAL4_MODE SL_LEGACY_SERIAL_UNUSED
#endif
...
#if defined(EMBER_MICRO_HAS_SC1) && ((SL_LEGACY_SERIAL1_MODE == SL_LEGACY_SERIAL_BUFFER) || defined(EMBER_SERIAL1_RTSCTS))
  #undef SL_LEGACY_SERIAL1_MODE
  #define SL_LEGACY_SERIAL1_MODE SL_LEGACY_SERIAL_FIFO
  #define EM_SER1_FIFO_DMA_USED
  #ifndef SL_LEGACY_SERIAL1_TX_QUEUE_SIZE
    #define SL_LEGACY_SERIAL1_TX_QUEUE_SIZE 128
    #define SL_LEGACY_SERIAL1_RX_QUEUE_SIZE 128
  #endif
#endif
#if defined(EMBER_MICRO_HAS_SC3) && ((SL_LEGACY_SERIAL2_MODE == SL_LEGACY_SERIAL_BUFFER) || defined(EMBER_SERIAL2_RTSCTS))
  #undef SL_LEGACY_SERIAL2_MODE
  #define SL_LEGACY_SERIAL2_MODE SL_LEGACY_SERIAL_FIFO
  #define EM_SER2_FIFO_DMA_USED
  #ifndef SL_LEGACY_SERIAL2_TX_QUEUE_SIZE
    #define SL_LEGACY_SERIAL2_TX_QUEUE_SIZE 128
    #define SL_LEGACY_SERIAL2_RX_QUEUE_SIZE 128
  #endif
#endif

#if (SL_LEGACY_SERIAL0_MODE == SL_LEGACY_SERIAL_FIFO)  \
  || (SL_LEGACY_SERIAL1_MODE == SL_LEGACY_SERIAL_FIFO) \
  || (SL_LEGACY_SERIAL2_MODE == SL_LEGACY_SERIAL_FIFO) \
  || (SL_LEGACY_SERIAL3_MODE == SL_LEGACY_SERIAL_FIFO) \
  || (SL_LEGACY_SERIAL4_MODE == SL_LEGACY_SERIAL_FIFO)
  #define EM_ENABLE_SERIAL_FIFO
#endif

#if (SL_LEGACY_SERIAL0_MODE == SL_LEGACY_SERIAL_BUFFER)  \
  || (SL_LEGACY_SERIAL3_MODE == SL_LEGACY_SERIAL_BUFFER) \
  || (SL_LEGACY_SERIAL4_MODE == SL_LEGACY_SERIAL_BUFFER)
...
extern uint16_t const emSerialTxQueueMasks[EM_NUM_SERIAL_PORTS];

/** Sizes used for managing the FIFOs of the transmit queues.*/
extern uint16_t const emSerialTxQueueSizes[EM_NUM_SERIAL_PORTS];

/** Sizes used for managing the FIFOs of the receive queues.*/
extern uint16_t const emSerialRxQueueSizes[EM_NUM_SERIAL_PORTS];

/** Last UART receive error to occur.*/
extern sl_status_t emSerialRxError[EM_NUM_SERIAL_PORTS];
...
extern uint8_t const emSerialPortModes[EM_NUM_SERIAL_PORTS];

extern uint16_t const emSerialTxQueueWraps[EM_NUM_SERIAL_PORTS];
extern uint16_t const emSerialRxQueueWraps[EM_NUM_SERIAL_PORTS];
...
void sli_zigbee_stack_call_populate_counters(sl_zigbee_counter_type_t type, uint8_t data);
  ...
  #define HANDLE_ASH_ERROR(type) sli_zigbee_stack_call_populate_counters(type, 0)
#else
 ...
 * otherise SL_STATUS_OK.
 */
sl_status_t halInternalUartInit(uint8_t port,
                                SerialBaudRate rate,
                                SerialParity parity,
                                uint8_t stopBits);
 ...
 *  It is called from sli_legacy_serial_read_byte(), and based on the number of bytes
 ...
 */
#if (SL_LEGACY_SERIAL1_MODE == SL_LEGACY_SERIAL_FIFO) \
  && (defined(SL_LEGACY_SERIAL1_XONXOFF))
 ...
 * by ::sli_legacy_serial_read_byte().  It is responsible for maintaining synchronization
