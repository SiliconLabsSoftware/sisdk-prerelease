...
#if     !(defined(SL_ZIGBEE_TEST) || defined(SL_ZIGBEE_NO_STACK))
#warning base/hal/hal.h utilized?!
#endif//!(defined(SL_ZIGBEE_TEST) || defined(SL_ZIGBEE_NO_STACK))
...
#define BSP_SERIAL_APP_PORT APP_SERIAL
  ...
  #if defined(UNIX_HOST) && !defined(SL_ZIGBEE_TEST)
...
// TODO: here we include only the functionalities that we will have on mustang
    ...
    #endif //CORTEXM3
    #include "micro/system-timer.h"
    #if defined(MAC_TEST_STACK) && defined(SL_ZIGBEE_TEST)
    #include "phy/symbol-timer.h"
    #endif
    #include "micro/spi.h"
    #if defined(CORTEXM3) || defined(SL_ZIGBEE_TEST)
      #include "micro/serial.h"
    #else
    ...
    #include "micro/token.h"
      ...
      #include "micro/led.h"
    #endif
  #endif // UNIX_HOST && !SL_ZIGBEE_TEST
...
//    #include "micro/bootloader-eeprom.h"
  ...
  #include "micro/token.h"
    ...
    #include "micro/led.h"
  #endif
#elif !defined(SL_ZIGBEE_STACK_IP)
// Pro Stack
  #include "micro/button.h"
  #include "micro/crc.h"
  #include "micro/endian.h"
...
//Host processors sometimes use the token module.
  #if !defined(EZSP_HOST) || defined(SL_ZIGBEE_AF_API_TOKEN)
    #include "micro/token.h"
  #endif
...
//Host processors sometimes use the token module.
  #if !defined(UNIX_HOST) || defined(SL_ZIGBEE_AF_API_TOKEN)
    #include "micro/token.h"
  #endif
