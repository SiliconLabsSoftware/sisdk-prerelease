...
static sl_zigbee_af_dido_storage_status_t setDidoStorageDevice(const char* directoryOrFile);
static sl_zigbee_af_dido_storage_status_t createDefaultStorageDirectory(void);
static char *getNewDidoReportFilepath(void);
static sl_zigbee_af_dido_storage_status_t writeRawData(const char* filepath,
                                                       const uint8_t* data,
                                                       uint32_t length);
...
sl_zigbee_af_dido_storage_status_t sl_zigbee_af_dido_storage_write_report(uint8_t* data,
                                                                          uint32_t length)
{
  sl_zigbee_af_dido_storage_status_t status;
  char *reportFilepath;
  ...
  if (reportFilepath == NULL) {
    return SL_ZIGBEE_AF_DIDO_STORAGE_ERROR;
  }
...
static sl_zigbee_af_dido_storage_status_t setDidoStorageDevice(const char* directoryOrFile)
{
  ...
  (void) memset(storageDevice, 0, length + 2);
  ...
  initDone = true;
  return SL_ZIGBEE_AF_DIDO_STORAGE_SUCCESS;
}
...
static char *getNewDidoReportFilepath()
{
  ...
  sl_zigbee_af_dido_storage_status_t status = SL_ZIGBEE_AF_DIDO_STORAGE_ERROR;
                                                       ...
                                                       uint32_t length)
{
  ...
  if (written != length) {
    ...
    status = SL_ZIGBEE_AF_DIDO_STORAGE_ERROR;
  } else {
    status = SL_ZIGBEE_AF_DIDO_STORAGE_SUCCESS;
  }
  fclose(fileHandle);
  return status;
}
