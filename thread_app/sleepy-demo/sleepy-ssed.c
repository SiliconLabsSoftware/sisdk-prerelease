...
void sleepyInit(void)
{
    ...
    SuccessOrExit(error = otLinkSetCslChannel(otInstanceGetSingle(), SL_OPENTHREAD_DEFAULT_DATASET_CHANNEL));
    SuccessOrExit(error = otLinkSetCslPeriod(otInstanceGetSingle(), SSED_CSL_PERIOD_US));
    SuccessOrExit(error = otLinkSetCslTimeout(otInstanceGetSingle(), SSED_CSL_TIMEOUT_SEC));
...
void initUdp(void)
{
    ...
    // Open the socket
    error = otUdpOpen(otInstanceGetSingle(), &sSsedSocket, ssedReceiveCallback, NULL);
    if (error != OT_ERROR_NONE)
    {
    ...
    error = otUdpBind(otInstanceGetSingle(), &sSsedSocket, &bindAddr, OT_NETIF_THREAD_INTERNAL);
    if (error != OT_ERROR_NONE)
    {
        otCliOutputFormat("SSED failed to bind udp socket with: %d, %s\r\n", error, otThreadErrorToString(error));
        IgnoreReturnValue(otUdpClose(otInstanceGetSingle(), &sSsedSocket));
        return;
    }
}
...
void applicationTick(void)
{
    ...
    if (sButtonPressed)
    {
        ...
        SuccessOrExit(otUdpSend(otInstanceGetSingle(), &sSsedSocket, message, &messageInfo));
...
void ssedReceiveCallback(void *aContext, otMessage *aMessage, const otMessageInfo *aMessageInfo)
{
    const void      *context = aContext;
    const otMessage *message = aMessage;
