...
{
  (void)sl_rtl_cs_deinit(&inst->libitem);
...
static void sli_rtl_service_step_cs(void)
{
  ...
  for (unsigned i = 0; i < SL_RTL_SERVICE_CS_MAX_INSTANCES; i++) {
    ...
    if (inst->request_type == SLI_REQUEST_CS) {
      rc = sl_rtl_cs_process(&inst->libitem, 1, inst->pending_procedure.cs);
    } else {
      rc = sl_rtl_ras_process(&inst->libitem, 1, inst->pending_procedure.ras);
    }
                                              ...
                                              sl_rtl_service_cs_ctx_t         **ctx_out)
{
  ...
  if (s_rta_ctx_handle == APP_RTA_INVALID_CONTEXT) {
    return SL_RTL_ERROR_NOT_INITIALIZED;
  }
                                                         ...
                                                         sl_rtl_service_cs_inst_t **inst_out)
{
  ...
  enum sl_rtl_error_code rc = sl_rtl_cs_init(&slot->libitem);
...
{
  if ((inst == NULL) || !inst->in_use || (inst->parent == NULL)) {
    return SL_RTL_ERROR_ARGUMENT;
  }
                                                       ...
                                                       sl_rtl_cs_algo_mode        mode)
{
  ...
  enum sl_rtl_error_code rc = sl_rtl_cs_set_algo_mode(&inst->libitem, mode);
  (void)app_rta_release(ctx->rta_ctx);
  return rc;
}
                                                  ...
                                                  sl_rtl_cs_mode             sub_mode)
{
  ...
  enum sl_rtl_error_code rc = sl_rtl_cs_set_cs_mode(&inst->libitem, main_mode, sub_mode);
  (void)app_rta_release(ctx->rta_ctx);
  return rc;
}
...
{
  if ((inst == NULL) || !inst->in_use || (params == NULL)) {
    return SL_RTL_ERROR_ARGUMENT;
  }
  sl_rtl_service_cs_ctx_t *ctx = inst->parent;
  if (app_rta_acquire(ctx->rta_ctx) != SL_STATUS_OK) {
  ...
  enum sl_rtl_error_code rc = sl_rtl_cs_set_cs_params(&inst->libitem, params);
  (void)app_rta_release(ctx->rta_ctx);
  return rc;
}
...
enum sl_rtl_error_code sl_rtl_service_create_cs_estimator(sl_rtl_service_cs_inst_t *inst)
{
  ...
  enum sl_rtl_error_code rc = sl_rtl_cs_create_estimator(&inst->libitem);
  (void)app_rta_release(ctx->rta_ctx);
  return rc;
}
...
{
  if ((inst == NULL) || !inst->in_use || (param == NULL)) {
    return SL_RTL_ERROR_ARGUMENT;
  }
  sl_rtl_service_cs_ctx_t *ctx = inst->parent;
  if (app_rta_acquire(ctx->rta_ctx) != SL_STATUS_OK) {
  ...
  enum sl_rtl_error_code rc = sl_rtl_cs_set_estimator_param(&inst->libitem, param);
  (void)app_rta_release(ctx->rta_ctx);
  return rc;
}
...
{
  if ((inst == NULL) || !inst->in_use || (procedure == NULL)) {
    return SL_RTL_ERROR_ARGUMENT;
  }
  sl_rtl_service_cs_ctx_t *ctx = inst->parent;
  ...
  float                                *distance)
{
  ...
  return sl_rtl_cs_get_distance_estimate(sli_result_libitem(result),
                                         estimate_type,
                                         estimate_mode,
                                         distance);
}
  ...
  float                                      *confidence)
{
  ...
  return sl_rtl_cs_get_distance_estimate_confidence(sli_result_libitem(result),
                                                    confidence_type,
                                                    confidence_mode,
                                                    confidence);
}
  ...
  float                                         *extended_info)
{
  ...
  return sl_rtl_cs_get_distance_estimate_extended_info(sli_result_libitem(result),
                                                       extended_info_type,
                                                       extended_info);
}
