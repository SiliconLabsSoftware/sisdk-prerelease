...
/// CLI option socket buffer size
#define SL_IPERF_CLI_OPT_SOCKET_BUFFER_SIZE           "socket_buffer_size"
...
// socket buffer size
/**************************************************************************//**
 * @brief Socket buffer size setter
 * @param[in] val pointer to the buffer size (uint32_t)
 *****************************************************************************/
__STATIC_INLINE void _opt_socket_buffer_size_setter(void *val);

/**************************************************************************//**
 * @brief Socket buffer size getter
 * @return string buffer ptr
 *****************************************************************************/
__STATIC_INLINE const char *_opt_socket_buffer_size_getter(void);
...
static sl_iperf_opt_t _options = {
  ...
  .socket_buffer_size = SL_IPERF_SOCKET_BUFFER_SIZE
};
...
static sl_iperf_cli_property_t _iperf_properties[] = {
  ...
  {
    .domain = SL_IPERF_CLI_DOMAIN_OPTIONS,
    .name   = SL_IPERF_CLI_OPT_SOCKET_BUFFER_SIZE,
    .type   = SL_IPERF_CLI_ARG_TYPE_UINT32,
    .setter = _opt_socket_buffer_size_setter,
    .getter = _opt_socket_buffer_size_getter,
    .help   = SL_IPERF_CLI_OPT_SOCKET_BUFFER_SIZE,
    .flags  = SL_IPERF_CLI_OPT_FLAGS_UDP_MASK
              | SL_IPERF_CLI_OPT_FLAGS_CLNT_MASK
  },
  {
...
void sl_iperf_cli_set(const sl_cli_command_arg_t *arguments)
{
  ...
  const char *arg1_str                 = NULL;
  char *domain                         = NULL;
  char *key                            = NULL;
  const sl_iperf_cli_property_t * prop = NULL;
  int64_t i_val                        = 0LL;
...
__STATIC_INLINE void _opt_remote_address_setter(void *val)
{
  sl_strcpy_s(_options.remote_addr, SL_IPERF_IP_STR_BUFF_LEN, val);
}
...
__STATIC_INLINE void _opt_buffer_length_setter(void *val)
{
  ...
  if (!value || value > _options.socket_buffer_size) {
...
__STATIC_INLINE void _opt_socket_buffer_size_setter(void *val)
{
  ...
  val32 = *(uint32_t*)val;
  if (val32 > UINT16_MAX) {
    printf("[Socket buffer size value is overflowed]\n");
  } else {
    _options.socket_buffer_size = val32;
  }
}

__STATIC_INLINE const char *_opt_socket_buffer_size_getter(void)
{
  __prepare_getter_buff("%lu bytes", _options.socket_buffer_size);
}
                       ...
                       const sl_iperf_protocol_t protocol)
{
  ...
  // init test
  sl_iperf_test_init(&test, _options.mode, _options.protocol, _options.buf_len);
                            ...
                            char **key_dst)
{
  ...
  sl_strcpy_s(domain_key_buff, SL_IPERF_CLI_MAX_STR_ARG_LEN, str_src);
