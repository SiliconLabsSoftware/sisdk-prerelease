...
#if !defined(SL_CATALOG_WISUN_APP_SETTING_PRESENT)
/// Wi-SUN Traffic params structure
typedef struct app_params_traffic {
  ...
  /// Allow EDFE for non fragmented packets
  bool allow_edfe_all_packets;
} app_params_traffic_t;
...
// -----------------------------------------------------------------------------
static sl_status_t app_set_options(const app_setting_wisun_t * const setting)
{
...
#if defined(WISUN_CONFIG_BROADCAST_RETRIES)
    { SL_WISUN_OPTION_MPL_TRICKLE_E,                   &broadcast_retries,                  sizeof(broadcast_retries) },
#endif
    { SL_WISUN_OPTION_MAC_MIN_BE,                      &setting->mac.min_be,                      sizeof(setting->mac.min_be) },
    { SL_WISUN_OPTION_MAC_MAX_BE,                      &setting->mac.max_be,                      sizeof(setting->mac.max_be) },
    { SL_WISUN_OPTION_MAC_BACKOFF_PERIOD_US,           &setting->mac.backoff_period_us,           sizeof(setting->mac.backoff_period_us) },
    { SL_WISUN_OPTION_MAC_MAX_CCA_RETRIES,             &setting->mac.max_cca_retries,             sizeof(setting->mac.max_cca_retries) },
    { SL_WISUN_OPTION_MAC_MAX_FRAME_RETRIES,           &setting->mac.max_frame_retries,           sizeof(setting->mac.max_frame_retries) },
    { SL_WISUN_OPTION_TRAFFIC_LOWPAN_MTU_BYTES,        &setting->traffic.lowpan_mtu,              sizeof(setting->traffic.lowpan_mtu) },
    { SL_WISUN_OPTION_TRAFFIC_IPV6_MRU_BYTES,          &setting->traffic.ipv6_mru,                sizeof(setting->traffic.ipv6_mru) },
    { SL_WISUN_OPTION_TRAFFIC_MAX_EDFE_FRAGMENT_COUNT, &setting->traffic.max_edfe_fragment_count, sizeof(setting->traffic.max_edfe_fragment_count) },
    { SL_WISUN_OPTION_TRAFFIC_ALLOW_EDFE_ALL_PACKETS,  &setting->traffic.allow_edfe_all_packets,  sizeof(setting->traffic.allow_edfe_all_packets) },
  };
  sl_status_t ret;
