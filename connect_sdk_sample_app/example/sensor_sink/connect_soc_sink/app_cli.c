...
void cli_pjoin(sl_cli_command_arg_t *arguments)
{
  ...
  if (sl_cli_get_argument_count(arguments) > 1) {
    ...
    if (status != EMBER_SUCCESS) {
      app_log_info("Set selective join payload status: 0x%02" PRIX8, status);
    }
  } else {
  ...
  if (status != EMBER_SUCCESS) {
    app_log_info("Permit join status: 0x%02" PRIX8, status);
  }
}
...
void cli_remove_child(sl_cli_command_arg_t *arguments)
{
  ...
  app_log_info("Child removal 0x%02" PRIX8 "\n", status);
}
...
void cli_counter(sl_cli_command_arg_t *arguments)
{
  ...
  if (status == EMBER_SUCCESS) {
    app_log_info("Counter type=%" PRIu8 ": %" PRIu32 "\n", counter_type, counter);
  } else {
