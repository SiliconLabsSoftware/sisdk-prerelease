/*******************************************************************************
 * @file
 * @brief OpenThread watchdog crash catch implementation
 *******************************************************************************
 * # License
...
#include "watchdog_crash_catch.h"
#include "sl_component_catalog.h"
#include "sl_openthread_watchdog_config.h"

// CORTEXM3 is the legacy HAL name for all device_cortexm parts (M33, M55, etc.).
#if (SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_ENABLE != 0) && defined(CORTEXM3)
...
#define SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_LOG(...) otLogCritPlat(__VA_ARGS__)
#else
#define SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_LOG(...)
#endif

#if defined(SL_CATALOG_CMSIS_OS_COMMON_PRESENT)
#define SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_FREERTOS 1
#else
#define SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_FREERTOS 0
#endif
...
#define SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_XPSR_THUMB_BIT 0x01000000U

// Max bytes to scan below PSP when searching for a preempted task exception frame.
#define SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_PSP_SCAN_BYTES 0x2000U

// ARMv7-M / ARMv8-M EXC_RETURN pattern (Cortex-M3/M4/M33/M55): 0xFFFFFF00–0xFFFFFFEF.
#define SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXC_RETURN_MASK 0xFFFFFF00U
#define SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXC_RETURN_LOW_MASK 0x0000000FU
#define SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXC_RETURN_THREAD 0xFFFFFFFDU
#define SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXC_RETURN_HANDLER 0xFFFFFFF9U

// Standard hardware exception frame: R0–R3, R12, LR, PC, xPSR (8 words).
#define SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXCEPTION_FRAME_WORDS 8U
// Stacked PC word index (same as halInternalCrashHandler sp += 6).
#define SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXCEPTION_FRAME_PC_INDEX 6U
...
{
    return ((lr & SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXC_RETURN_MASK)
            == SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXC_RETURN_MASK)
           && ((lr & SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXC_RETURN_LOW_MASK) != 0U);
}
...
static bool is_frame_index_bounds_ok(const uint32_t *frame)
{
    ...
    const uintptr_t frameEnd =
        frameBegin + (SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXCEPTION_FRAME_WORDS * sizeof(uint32_t));
    const uintptr_t pcWordAddr =
        frameBegin + (SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXCEPTION_FRAME_PC_INDEX * sizeof(uint32_t));
...
static bool is_valid_msp_exception_frame(const uint32_t *frame)
{
    ...
    if (!is_text_thumb_pc(frame[SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXCEPTION_FRAME_PC_INDEX]))
    {
        return false;
    }

    if (frame[SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXCEPTION_FRAME_WORDS - 1U] == 0U)
    {
        return false;
    }

    if ((frame[SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXCEPTION_FRAME_WORDS - 1U]
         & SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_XPSR_THUMB_BIT)
        == 0U)
    {
        return false;
    }
...
static bool is_valid_psp_exception_frame(const uint32_t *frame)
{
    ...
    return is_text_thumb_pc(frame[SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXCEPTION_FRAME_PC_INDEX]);
}
...
static const uint32_t *find_exception_frame_below(uintptr_t spLimit, uintptr_t stackLow)
{
    ...
    const uintptr_t frameSize = SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXCEPTION_FRAME_WORDS * sizeof(uint32_t);
...
static const uint32_t *find_psp_exception_frame(uintptr_t pspTop)
{
    ...
    if (pspTop > SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_PSP_SCAN_BYTES)
    {
        pspLow = pspTop - SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_PSP_SCAN_BYTES;
    }
...
static const uint32_t *find_preempted_exception_frame(uint32_t msp, uint32_t psp)
{
    ...
    if (SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_FREERTOS != 0)
    {
...
static void capture_probable_returns(HalCrashInfoType *c, const uint32_t *sEnd, const uint32_t *stackTop)
{
    uint8_t         i             = 0U;
    uint8_t         j             = 0U;
    uint32_t        data          = 0U;
...
{
    return (SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_FREERTOS != 0) ? SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXC_RETURN_THREAD
                                                              : SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXC_RETURN_HANDLER;
}
...
{
    return (SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_FREERTOS != 0) && ((c->LR & 4U) != 0U);
}
...
static void load_crash_info_from_frame(HalCrashInfoType *c, const uint32_t *frame)
{
    ...
    c->PC        = frame[SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXCEPTION_FRAME_PC_INDEX];
    c->xPSR.word = frame[SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXCEPTION_FRAME_WORDS - 1U];
...
static void setup_stack_scan_window(HalCrashInfoType *c,
                                    const uint32_t   *sp,
                                    const uint32_t  **stackBottom,
                                    const uint32_t  **stackTop)
{
    if (crash_uses_process_stack(c))
    {
        *stackBottom     = sp;
        *stackTop        = sp + SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_EXCEPTION_FRAME_WORDS;
...
static const uint32_t *expanded_scan_top(const uint32_t         *frame,
                                         uint32_t                psp,
                                         const HalCrashInfoType *c,
                                         const uint32_t         *stackTop)
{
    ...
    {
        const uintptr_t expandedTop = (uintptr_t)psp + SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_PSP_SCAN_BYTES;
...
static const uint32_t *adjust_stack_scan_end(const uint32_t   *frame,
                                             HalCrashInfoType *c,
                                             const uint32_t   *sp,
                                             const uint32_t   *stackBottom,
                                             const uint32_t   *stackTop,
                                             const uint32_t   *sEnd)
{
    ...
    const bool outsideStack = (topAddress < bottomAddress) || (spAddress < bottomAddress) || (spAddress > topAddress)
                              || ((topAddress - spAddress) < frameSize);
...
static void capture_stack_returns_and_pc(HalCrashInfoType *c,
...
SL_CODE_CLASSIFY(SL_CODE_COMPONENT_PLATFORM, SL_CODE_CLASS_TIME_CRITICAL)
...
void sl_ot_watchdog_crash_catch_init(void)
{
    sl_status_t status = sl_watchdog_manager_set_starve_callback(starve_callback);

    if (status != SL_STATUS_OK)
    {
        SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_LOG(
            "Watchdog crash catch disabled: platform starve callback unsupported (0x%lx)",
            (unsigned long)status);
        return;
    }

    SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_LOG("Watchdog crash catch enabled (platform WM starve callback)");
}
...
void sl_ot_watchdog_crash_catch_init(void)
{
    SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_LOG("Watchdog crash catch unsupported: no WDOG_IF_WARN on this part");
}

#endif // WDOG_IF_WARN

#else // SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_ENABLE && CORTEXM3

void sl_ot_watchdog_crash_catch_init(void)
{
}

#endif // SL_OPENTHREAD_WATCHDOG_CRASH_CATCH_ENABLE && CORTEXM3
