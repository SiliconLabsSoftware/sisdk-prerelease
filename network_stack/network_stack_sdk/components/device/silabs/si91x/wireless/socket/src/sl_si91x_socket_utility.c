...
void sli_si91x_free_socket(int socket)
{
  ...
  sl_status_t status                            = sli_command_engine_remove_packet_type(
    command_engine_instance,
    (uint8_t)(sli_get_wifi_command_engine_max_packet_type_count() + si91x_socket->index));
...
sl_status_t sli_get_free_socket(sli_si91x_socket_t **socket, int *socket_fd)
{
  ...
  *socket_fd         = -1;
  ...
  if (command_engine_instance == NULL) {
    SL_DEBUG_LOG_V2(ERROR, "Command engine instance is NULL\r\n");
    free(new_socket);
    sli_si91x_sockets[socket_index] = NULL;
    osMutexRelease(sli_si91x_socket_mutex);
  ...
  status =
    sli_command_engine_add_packet_type(command_engine_instance,
                                       ((uint8_t)(sli_get_wifi_command_engine_max_packet_type_count() + socket_index)),
...
sl_status_t sli_si91x_socket_pre_tx_handler(sli_command_engine_t *instance, uint16_t packet_type, void *data)
{
  ...
  if (packet->command == SLI_WIFI_REQ_SOCKET_READ_DATA
      && packet_type >= sli_get_wifi_command_engine_max_packet_type_count()) {
    // Check if the socket is already closed
    // Extract socket index from packet_type: packet_type = socket->index + sizeof(sli_wifi_command_engine_packet_type_configuration) / sizeof(sli_command_engine_packet_type_configuration_t)
                                                ...
                                                const sli_si91x_socket_context_t *sdk_context)
{
  ...
  if (frame_status != SL_STATUS_OK) {
    ...
    // Get packet type configuration for the given command type
    status = sli_command_engine_get_rx_queue_info_from_packet_type(command_engine_instance,
...
sl_status_t sli_si91x_socket_data_event_handler(sl_wifi_buffer_t *rx_buffer)
{
  ...
  if (client_socket->is_receive_cmd_pending == true) {
    ...
    // Get packet type configuration for the given command type
    sl_status_t status =
      sli_command_engine_get_rx_queue_info_from_packet_type(command_engine_instance,
                                                            (uint16_t)(client_socket->index + SLI_SI91X_CMD_MAX),
                                                            &packet_type_info);
    if (status != SL_STATUS_OK) {
      sli_buffer_manager_free_buffer(rx_buffer);
