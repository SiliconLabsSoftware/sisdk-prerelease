/***************************************************************************//**
 * @file
 * @brief CS SoC RREQ initiator example - CS result handling
 *******************************************************************************
 * # License
                   ...
                   const cs_algo_result_t *ranging_data)
{
  ...
  if (result != NULL) {
    ...
    if (initiator == NULL) {
      log_error(APP_INSTANCE_PREFIX "Failed to get initiator instance in result callback!" NL,
                conn_handle);
      return;
    }
...
void app_print_head_and_data(initiator_instance_t *initiator)
{
  ...
  for (uint8_t is_data = ((measurement_counter % CS_HEADER_LOG) > 0); is_data <= 1; is_data++) {
