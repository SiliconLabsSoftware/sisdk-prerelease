/***************************************************************************//**
 * @file
 * @brief CS SoC RREQ initiator example - Channel Sounding (CS) logic
 *******************************************************************************
 * # License
...
// the single-instance (single-peer) result; the multiple-instance bounds are
// independent of the antenna path count.
...
void app_cs_check_supported_capabilities(const sl_bt_msg_t *evt)
{
  ...
  if (cs_config.cs_sync_phy == sl_bt_gap_phy_2m
    ...
    log_info(APP_PREFIX "Using CS SYNC PHY %d instead" NL,
...
static uint8_t get_algo_mode(void)
{
  static uint8_t latched_algo_mode = 0xFFu;
  ...
  if (latched_algo_mode == 0xFFu) {
    ...
    if ((SL_SIMPLE_BUTTON_COUNT > 1)
      ...
      latched_algo_mode = alternative_mode;
    } else {
      latched_algo_mode = (uint8_t)CS_ALGO_CONFIG_DEFAULT_ALGO_MODE;
    }
  }
  return latched_algo_mode;
}
#else
static uint8_t get_algo_mode(void)
{
  return (uint8_t)CS_ALGO_CONFIG_DEFAULT_ALGO_MODE;
}
#endif // SL_CATALOG_SIMPLE_BUTTON_PRESENT
                            ...
                            cs_algo_config_t *algo_config)
{
  ...
  log_info(APP_INSTANCE_PREFIX "Using tone antenna configuration index: %u" NL,
           conn_handle,
           antenna_config);
  // Adjust preferred antenna when remote uses a single antenna
  if ((procedure_parameters->preferred_peer_antenna == 3)
      && ((remote_antenna_count == 1)
          || (antenna_config == CS_ANTENNA_CONFIG_INDEX_DUAL_LOCAL_SINGLE_REMOTE)
          || (antenna_config == CS_ANTENNA_CONFIG_INDEX_SINGLE_ONLY))) {
    procedure_parameters->preferred_peer_antenna = 1;
  }
  log_info(APP_INSTANCE_PREFIX "Using preferred peer antenna: %u" NL,
           conn_handle,
...
void app_cs_set_default_connection_parameters(void)
{
  ...
  // scheduling and the configured CS Manager instance count. Passing
  // CS_MANAGER_CONFIG_MAX_INSTANCES as the peer count means optimize selects
  // the multiple-reflector bounds when more than one instance is configured,
  // and the single-peer bounds otherwise.
  cs_configurator_parameters_t configurator_parameters = {
    .cs_instance_config = &manager_instance_config,
    .cs_config = &cs_config,
    .rreq_config = &rreq_create_config,
  ...
  log_info(APP_PREFIX "Default connection interval set to %u (%u CS Manager "
                      "instance(s) configured)" NL,
           connection_parameters.max_connection_interval,
           CS_MANAGER_CONFIG_MAX_INSTANCES);
}
