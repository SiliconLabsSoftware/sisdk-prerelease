/***************************************************************************//**
 * @file
 * @brief CS SoC RREQ initiator example - CS error handling
 *******************************************************************************
 * # License
...
void app_on_cs_algo_on_error(uint8_t conn_handle,
                             ...
                             sl_status_t sc)
{
  // "Estimation in progress" is an expected in static mode, not an error
  if (error == CS_ALGO_ERROR_RTL_ESTIMATION_IN_PROGRESS) {
    return;
  }
  report(conn_handle,
         CS_ERROR_TYPE_ALGO,
         ...
         sc,
         algo_is_fatal(error));
  log_error(APP_INSTANCE_PREFIX "ranging counter (%u)" NL,
            conn_handle,
            ranging_counter);
}
