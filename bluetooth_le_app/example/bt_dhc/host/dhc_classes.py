...
# Get array sizes
RAIL_PA_NVM_CONFIGS = os.path.abspath(os.path.join((__file__), '../../../../../../platform/radio/rail_lib/plugin/pa-conversions/nvm/sl_rail_util_pa_nvm_configs.h'))
SL_RAIL_NVM_PA_CONFIGS_VERSION = None
SL_RAIL_NVM_PA_CURVE_SEGMENTS = None
SL_RAIL_NVM_PA_TABLE_ENTRIES = None

with open(RAIL_PA_NVM_CONFIGS, 'r') as f:
    # Regular expression to match #define statements
    define_pattern = re.compile(r'^\s*#define\s+(\w+)\s+(.*)$')

    for line in f:
        match = define_pattern.match(line)
        if match:
            if match.group(1) == "SL_RAIL_NVM_PA_CONFIGS_VERSION":
                SL_RAIL_NVM_PA_CONFIGS_VERSION = eval(match.group(2))
            if match.group(1) == "SL_RAIL_NVM_PA_CURVE_SEGMENTS":
                SL_RAIL_NVM_PA_CURVE_SEGMENTS = eval(match.group(2))
            if match.group(1) == "SL_RAIL_NVM_PA_TABLE_ENTRIES":
                SL_RAIL_NVM_PA_TABLE_ENTRIES = eval(match.group(2))

if SL_RAIL_NVM_PA_CONFIGS_VERSION is None:
    raise ReferenceError(f"SL_RAIL_NVM_PA_CONFIGS_VERSION cannot be found in {RAIL_PA_NVM_CONFIGS}")
if SL_RAIL_NVM_PA_CURVE_SEGMENTS is None:
    raise ReferenceError(f"SL_RAIL_NVM_PA_CURVE_SEGMENTS cannot be found in {RAIL_PA_NVM_CONFIGS}")
if SL_RAIL_NVM_PA_TABLE_ENTRIES is None:
    raise ReferenceError(f"SL_RAIL_NVM_PA_TABLE_ENTRIES cannot be found in {RAIL_PA_NVM_CONFIGS}")
