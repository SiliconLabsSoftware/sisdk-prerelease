...
sl_status_t on_event_security(const sl_bt_msg_t *evt)
{
  ...
  switch (SL_BT_MSG_ID(evt->header)) {
    case sl_bt_evt_system_boot_id:
      ...
      if (sc != SL_STATUS_OK) {
        app_log_error(APP_SEC_PREFIX "Failed to delete bondings, sc = %u" NL_SEC, sc);
      }
      #endif
      ...
      if (sc != SL_STATUS_OK) {
        app_log_error(APP_SEC_PREFIX "Failed to set debug mode, sc = %u" NL_SEC, sc);
      }
      #endif
      ...
      sc = sl_bt_sm_configure(sm_config_flags,
                              CS_CAPABILITY);
      if (sc != SL_STATUS_OK) {
        app_log_error(APP_SEC_PREFIX "Failed to configure security management, sc = %u" NL_SEC, sc);
        return sc;
      }
      // Set bondable mode
      sl_bt_sm_set_bondable_mode(ALLOW_BONDING);
      #if defined(NEW_BOND_REQUIRES_PASSKEY) && (NEW_BOND_REQUIRES_PASSKEY == 1)
      sc = sl_bt_sm_set_passkey(CS_PASSKEY);
      if (sc != SL_STATUS_OK) {
        app_log_error(APP_SEC_PREFIX "Failed to set passkey, sc = %u" NL_SEC, sc);
        return sc;
      }
      #endif
      ...
      sc = sl_bt_sm_delete_bondings();
      if (sc != SL_STATUS_OK) {
        app_log_error(APP_SEC_PREFIX "Failed to delete bonding DB, sc = %u" NL_SEC, sc);
      }
      else {
    ...
    case sl_bt_evt_sm_bonded_id:
      ...
      break;
        case sl_bt_evt_sm_bonding_failed_id:
              app_log_info(APP_SEC_PREFIX "Bonding failed on initiator side, reason: 0x%lx" NL_SEC, (unsigned long)evt->data.evt_sm_bonding_failed.reason);
          ...
          if (evt->data.evt_sm_bonding_failed.reason == SL_STATUS_BT_CTRL_PIN_OR_KEY_MISSING) {
              app_log_error(APP_SEC_PREFIX "Missing key. Deleting corresponding bonding info." NL_SEC);
              uint32_t bonding;
              bd_addr address;
              ...
              if (sc != SL_STATUS_OK) {
                app_log_error(APP_SEC_PREFIX "Failed to get remote address, sc = %x" NL_SEC, sc);
                return sc;
              }
              sc = sl_bt_sm_find_bonding_by_address(address, &bonding, &security_mode, &key_size);
              if (sc != SL_STATUS_OK) {
                app_log_error(APP_SEC_PREFIX "Failed to find bonding by address, sc = 0x%x" NL_SEC, sc);
                return sc;
              }
              sc= sl_bt_sm_delete_bonding((uint8_t)bonding);
              if (sc != SL_STATUS_OK) {
                app_log_error(APP_SEC_PREFIX "Failed to delete bonding DB, sc = 0x%x" NL_SEC, sc);
                return sc;
              }
              else {
    ...
    case sl_bt_evt_sm_passkey_display_id:
    {
      ...
      app_log_info(APP_SEC_PREFIX "Displaying passkey: %06u" NL_SEC,
    ...
    case sl_bt_evt_sm_passkey_request_id:
    {
      ...
      sc = sl_bt_sm_enter_passkey(evt->data.evt_sm_passkey_request.connection, CS_PASSKEY);
      app_log_info(APP_SEC_PREFIX "Passkey entered %d, sc = %u" NL_SEC, CS_PASSKEY, sc);
      if (sc != SL_STATUS_OK) {
        app_log_error(APP_SEC_PREFIX "Failed to enter passkey, sc = %u" NL_SEC, sc);
        return sc;
      }
    }
    break;
    case sl_bt_evt_sm_confirm_passkey_id:
    {
      ...
      app_log_info(APP_SEC_PREFIX "Confirmation needed for passkey %u:" NL_SEC, evt->data.evt_sm_confirm_passkey.passkey);
