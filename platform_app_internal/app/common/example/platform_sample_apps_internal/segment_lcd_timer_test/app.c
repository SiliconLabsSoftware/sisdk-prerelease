...
#if defined(_EFR32_FG_FAMILY)
#define BTN0_GPIO_PORT  SL_GPIO_PORT_B
#define BTN0_GPIO_PIN   1
#define BTN1_GPIO_PORT  SL_GPIO_PORT_A
#define BTN1_GPIO_PIN   5
#elif defined(_EFM32_PG_FAMILY)
#define BTN0_GPIO_PORT  SL_GPIO_PORT_A
#define BTN0_GPIO_PIN   5
#define BTN1_GPIO_PORT  SL_GPIO_PORT_B
#define BTN1_GPIO_PIN   4
#endif
...
#if defined(_SILICON_LABS_32B_SERIES_2_CONFIG_3) && defined(_EFR32_FG_FAMILY)
  // Interrupt triggered by PB1 falling edge
  else if (int_no == _GPIO_IF_EXTIF5_SHIFT) {
...
static void on_button_1_change(uint8_t int_no, void *ctx)
{
  ...
  && defined(_EFM32_PG_FAMILY))

  if (int_no == BTN1_GPIO_PIN) {
    sl_hal_letimer_enable(LETIMER0);  // Need LETIMER running to detect compare
                                      // mode
    sl_hal_letimer_start(LETIMER0);
    // If in compare mode set up
...
#if defined(SL_LCD_MODULE_CL010_1087)
