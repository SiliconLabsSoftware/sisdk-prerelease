...
extern const struct sx_trng_selftest *default_selftests;
 ...
 *
 * @pre - the sx_trng_init() must be called first.
 */
...
/** TRNG initialization after successful self-tests execution
 *
 * This function performs the self-test list provided by \p tests and, if tests
 * pass, it will configure the TRNG either with default configuration or with
 * the user-provided\p config, in a similar way to sx_trng_init().
 *
 * The \p tests array must be a NULL terminated array.
 *
 * @param[in,out] ctx TRNG context to be used in other operations
 * @param[in] config pointer to optional configuration. NULL to use default.
 * @param[in] tests pointer to list of tests.
 *
 * @return ::SX_OK
 * @return ::SX_ERR_INCOMPATIBLE_HW
