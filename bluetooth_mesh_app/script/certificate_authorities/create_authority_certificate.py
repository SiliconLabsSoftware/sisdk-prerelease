...
'''Certificate Authority generator for Bluetooth Mesh Certificate-Based Provisioning (CBP).

Create an EC key pair and a certificate for either the Central or an Intermediate Authority of the given level.
The resulting root certificate (PEM) is used by the Mesh provisioner; it is not flashed onto devices.
...
def main(level,
    ...
    if level == 0:
        ...
        # For intermediate certificate creation, verify the higher authority before
        # creating any output so a missing/expired parent does not leave a partial folder.
        if level == 1:
            prev_name = 'central'
        else:
