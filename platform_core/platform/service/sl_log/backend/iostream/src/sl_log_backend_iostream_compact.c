 ...
 *
 * This file implements a compact iostream backend that writes raw sl_log_event_t
 * structs to iostream. No text formatting is applied; each event
 * is sent as sizeof(sl_log_event_t) bytes. This reduces code size and CPU
 * compared to the formatted iostream backend (sl_log_backend_iostream_formatted.c)
 * when a host or tool will decode the binary stream.
 *
 * Output: binary stream of sl_log_event_t. Layout follows sl_log_event_t
 * (see sl_log.h). Decoders must use the same struct definition and packing.
 *
 *******************************************************************************
 * # License
...
sl_status_t sl_log_hal_backend_write(const sl_log_event_t *buffer, uint32_t read_index, uint32_t event_count)
{
  ...
  if (event_count <= SL_LOG_NUMBER_OF_EVENTS - read_index) {
    /* contiguous block */
    status = sl_iostream_write(SL_LOG_IOSTREAM_HANDLE, (const void *)&buffer[read_index], event_count * sizeof(sl_log_event_t));
    return status;
  } else {
    /* first chunk: from read_index to end */
    uint32_t first_chunk = SL_LOG_NUMBER_OF_EVENTS - read_index;
    status = sl_iostream_write(SL_LOG_IOSTREAM_HANDLE, (const void *)&buffer[read_index], first_chunk * sizeof(sl_log_event_t));
    ...
    status = sl_iostream_write(SL_LOG_IOSTREAM_HANDLE, (const void *)buffer, second_chunk * sizeof(sl_log_event_t));
    return status;
  }
}
