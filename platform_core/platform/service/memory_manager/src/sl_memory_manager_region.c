...
sl_memory_region_t sl_memory_get_psram_heap_region(void)
{
  ...
  // Report the actual heap region.
  region.addr = (void *) PSRAM_BASE;
  region.size = PSRAM_SIZE;
...
sl_memory_region_t sl_memory_get_dtcm_heap_region(void)
{
  ...
  // Report the actual heap region.
#if defined(__GNUC__)
  extern char __vma_dtcm_start__[];
  extern char __vma_dtcm_end__[];

  region.addr = __vma_dtcm_end__;
  region.size = ((uintptr_t)__vma_dtcm_start__ + CORE_DTCM_SIZE) - (uintptr_t)__vma_dtcm_end__;

#elif defined(__ICCARM__)
  region.addr = (void *)CORE_NS_DTCM_MEM_BASE;
  region.size = CORE_DTCM_SIZE;
#endif
