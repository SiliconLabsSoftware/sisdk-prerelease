...
static uint32_t bits_to_value(const uint8_t *ptr,
                              size_t bit_offset,
                              size_t bit_count)
{
  uint32_t value = 0;
  size_t full_octets = bit_offset / 8;
  ptr += full_octets;
  bit_offset %= 8;

  while (bit_count) {
    value <<= 1;
    value |= (*ptr & (1 << (7 - bit_offset))) ? 1 : 0;
    if (bit_offset < 7) {
      bit_offset++;
    } else {
      bit_offset = 0;
                                       ...
                                       psa_key_id_t *ptr)
{
  ...
  return psa_status_to_sl_status(psa_import_key(&attributes, data, data_len, ptr));
}
                        ...
                        uint8_t *result)
{
  ...
  sc = psa_status_to_sl_status(psa_mac_sign_setup(&operation, key, alg));
  if (sc) {
    goto out;
  }

  sc = psa_status_to_sl_status(psa_mac_update(&operation, text_ptr, text_len));
  if (sc) {
    goto out;
  }

  sc = psa_status_to_sl_status(psa_mac_sign_finish(&operation, result, hash_len, &result_len));
  if (sc) {
    goto out;
  }
                                       ...
                                       uint8_t *result)
{
  ...
  e = psa_status_to_sl_status(psa_cipher_encrypt_setup(&operation, k, PSA_ALG_ECB_NO_PADDING));
  if (e) {
    goto out;
  }

  e = psa_status_to_sl_status(psa_cipher_update(&operation, text_ptr, 16, result, 16, &result_len));
  if (e) {
    goto out;
  }

  e = psa_status_to_sl_status(psa_cipher_finish(&operation, result + result_len, 16 - result_len, &result_len));
  if (e) {
    goto out;
  }
                                       ...
                                       size_t text_len)
{
  ...
  size_t out_len = 0;
  sc = psa_status_to_sl_status(psa_aead_decrypt(k,
                                                alg,
                                                nonce,
                                                13,
                                                additional_ptr,
...
{
  net_pdu->ivi = sl_mesh_net_pdu_ivi(pdu);
  net_pdu->nid = sl_mesh_net_pdu_nid(pdu);
  net_pdu->ctl = sl_mesh_net_pdu_ctl(pdu);
  net_pdu->ttl = sl_mesh_net_pdu_ttl(pdu);
  net_pdu->seq = sl_mesh_net_pdu_seq(pdu);
  net_pdu->src = sl_mesh_net_pdu_src(pdu);
  net_pdu->dst = sl_mesh_net_pdu_dst(pdu);
  net_pdu->trans_pdu_len = sl_mesh_net_pdu_data_len(pdu);
  net_pdu->transport_pdu = sl_mesh_net_pdu_data_ptr(pdu);
  net_pdu->net_mic_len = sl_mesh_net_pdu_mic_len(pdu);
  net_pdu->net_mic = sl_mesh_net_pdu_mic_ptr(pdu);
}

void sl_mesh_trans_pdu_data(const struct sl_mesh_net_pdu *pdu,
                            trans_pdu_t *trans_pdu, uint8_t ctl)
{
  trans_pdu->seg = sl_mesh_trans_pdu_seg(pdu);
  if (ctl == 1) { // Control message
    trans_pdu->opcode = sl_mesh_trans_pdu_control_opcode(pdu);
  } else {
    trans_pdu->akf = sl_mesh_trans_pdu_access_akf(pdu);
  }
  trans_pdu->aid = sl_mesh_trans_pdu_access_aid(pdu);
  trans_pdu->segment = sl_mesh_trans_pdu_data_ptr(pdu);
  trans_pdu->segment_len = sl_mesh_trans_pdu_data_len(pdu);
  if (trans_pdu->seg == 1) { // Segmented message
    if (ctl != 1) {
      trans_pdu->szmic = sl_mesh_trans_pdu_access_szmic(pdu);
    }
    trans_pdu->seq_0 = sl_mesh_trans_pdu_seg_seq0(pdu);
    trans_pdu->seg_o = sl_mesh_trans_pdu_seg_o(pdu);
    trans_pdu->seg_n = sl_mesh_trans_pdu_seg_n(pdu);
  } else {
    trans_pdu->segment = sl_mesh_trans_pdu_data_ptr(pdu);
    trans_pdu->segment_len = sl_mesh_trans_pdu_data_len(pdu);
  }
...
void sl_mesh_advertisement_header_from_buf(sl_bt_msg_t *evt, advertisement_header_t *adv_header)
{
  size_t pos = 0;
  uint8_t idx = 0;
  adv_header->pdu_type = evt->data.evt_scanner_legacy_advertisement_report.event_flags;
  switch (evt->data.evt_scanner_legacy_advertisement_report.event_flags) {
    // Connectable and scannable
    case (SL_BT_SCANNER_EVENT_FLAG_CONNECTABLE | SL_BT_SCANNER_EVENT_FLAG_SCANNABLE):
      adv_header->pdu_type = ADV_IND;
      break;
    // Connectable and directed
    case (SL_BT_SCANNER_EVENT_FLAG_CONNECTABLE | SL_BT_SCANNER_EVENT_FLAG_DIRECTED):
      adv_header->pdu_type = ADV_DIRECT_IND;
      break;
    // Non-connectable and non-scannable
    case 0x00:
      adv_header->pdu_type = ADV_NONCONN_IND;
      break;
    // Scannable and non-connectable
    case (SL_BT_SCANNER_EVENT_FLAG_SCANNABLE):
      adv_header->pdu_type = ADV_SCAN_IND;
      break;
    default:
      adv_header->pdu_type = 0xFF; // Unknown type
      break;
  }
  adv_header->tx_addr = evt->data.evt_scanner_legacy_advertisement_report.address;
  adv_header->tx_addr_type = evt->data.evt_scanner_legacy_advertisement_report.address_type;
  adv_header->rx_addr_type = evt->data.evt_scanner_legacy_advertisement_report.target_address_type;

  while (pos < evt->data.evt_scanner_legacy_advertisement_report.data.len) {
    if (idx >= MAX_ADV_DATA_NUM) {
      break;
    }

    adv_header->adv_data[idx].ad_len = evt->data.evt_scanner_legacy_advertisement_report.data.data[pos++];
    if (pos + adv_header->adv_data[idx].ad_len > evt->data.evt_scanner_legacy_advertisement_report.data.len) {
      break;
    }
    if (adv_header->adv_data[idx].ad_len >= 1) {
      adv_header->adv_data[idx].ad_type = evt->data.evt_scanner_legacy_advertisement_report.data.data[pos++];
      adv_header->adv_data[idx].data = evt->data.evt_scanner_legacy_advertisement_report.data.data + pos;
    }
    // The Length field includes ad_type, so subtract 1 to get the actual data length
    pos = pos + adv_header->adv_data[idx].ad_len - 1;
    idx++;
  }
  adv_header->data_num = idx;
}

void sl_mesh_segack_from_buf(const uint8_t *ptr, segack_t *segack)
{
  segack->obo = bits_to_value(ptr, 0, 1);
  segack->seq_zero = bits_to_value(ptr, 1, 13);
  segack->rfu = bits_to_value(ptr, 14, 2);
  segack->block_ack = bits_to_value(ptr, 16, 32);
}

void sl_mesh_friend_poll_from_buf(const uint8_t *ptr, friend_poll_t *friend_poll)
{
  friend_poll->padding = bits_to_value(ptr, 0, 7);
  friend_poll->fsn = bits_to_value(ptr, 7, 1);
}

void sl_mesh_friend_update_from_buf(const uint8_t *ptr, friend_update_t *friend_update)
{
  friend_update->rfu = bits_to_value(ptr, 0, 6);
  friend_update->ivu = bits_to_value(ptr, 6, 1);
  friend_update->kr = bits_to_value(ptr, 7, 1);
  friend_update->ivi = bits_to_value(ptr, 8, 32);
  friend_update->md = bits_to_value(ptr, 40, 8);
}

void sl_mesh_friend_request_from_buf(const uint8_t *ptr, friend_request_t *friend_request)
{
  friend_request->rfu = bits_to_value(ptr, 0, 1);
  friend_request->rssi_factor = bits_to_value(ptr, 1, 2);
  friend_request->window_factor = bits_to_value(ptr, 3, 2);
  friend_request->min_queue = bits_to_value(ptr, 5, 3);
  friend_request->receive_delay = bits_to_value(ptr, 8, 8);
  friend_request->poll_timeout = bits_to_value(ptr, 16, 24);
  friend_request->prev_addr = bits_to_value(ptr, 40, 16);
  friend_request->num_elements = bits_to_value(ptr, 56, 8);
  friend_request->lpn_counter = bits_to_value(ptr, 64, 16);
}

void sl_mesh_friend_offer_from_buf(const uint8_t *ptr, friend_offer_t *friend_offer)
{
  friend_offer->receive_window = bits_to_value(ptr, 0, 8);
  friend_offer->queue_size = bits_to_value(ptr, 8, 8);
  friend_offer->subs_list = bits_to_value(ptr, 16, 8);
  friend_offer->rssi = bits_to_value(ptr, 24, 8);
  friend_offer->friend_counter = bits_to_value(ptr, 32, 16);
}

void sl_mesh_friend_clear_from_buf(const uint8_t *ptr, friend_clear_t *friend_clear)
{
  friend_clear->lpn_address = bits_to_value(ptr, 0, 16);
  friend_clear->lpn_counter = bits_to_value(ptr, 16, 16);
}

void sl_mesh_friend_subscription_from_buf(const uint8_t *ptr, friend_subscription_t *friend_subscription)
{
  friend_subscription->tid = bits_to_value(ptr, 0, 8);
  friend_subscription->addr = (uint16_t*)(ptr + 8);
}

void sl_mesh_heartbeat_from_buf(const uint8_t *ptr, heartbeat_t *heartbeat)
{
  heartbeat->rfu = bits_to_value(ptr, 0, 1);
  heartbeat->ttl = bits_to_value(ptr, 1, 7);
  heartbeat->features = bits_to_value(ptr, 8, 16);
}

void sl_mesh_path_request_from_buf(const uint8_t *ptr, path_request_t *path_request)
{
  path_request->obo_dep = bits_to_value(ptr, 0, 1);
  path_request->ptype = bits_to_value(ptr, 1, 3);
  path_request->plife = bits_to_value(ptr, 4, 2);
  path_request->pdisc = bits_to_value(ptr, 6, 1);
  path_request->prohib = bits_to_value(ptr, 7, 1);
  path_request->fwdnum = bits_to_value(ptr, 8, 8);
  path_request->metric = bits_to_value(ptr, 16, 8);
  path_request->pdst = bits_to_value(ptr, 24, 16);
  path_request->prange_len =  bits_to_value(ptr, 40, 1) ? 24 : 16;
  path_request->prange = bits_to_value(ptr, 40, path_request->prange_len);
  path_request->drange_len = path_request->obo_dep ? (bits_to_value(ptr, 40 + path_request->prange_len, 1) ? 24 : 16) : 0;
  path_request->drange = path_request->obo_dep ? bits_to_value(ptr, 40 + path_request->prange_len, path_request->drange_len) : 0;
}

void sl_mesh_path_reply_from_buf(const uint8_t *ptr, path_reply_t *path_reply)
{
  path_reply->uni_dst = bits_to_value(ptr, 0, 1);
  path_reply->obo_dep = bits_to_value(ptr, 1, 1);
  path_reply->confirm = bits_to_value(ptr, 2, 1);
  path_reply->prohib = bits_to_value(ptr, 5, 3);
  path_reply->path_origin = bits_to_value(ptr, 8, 16);
  path_reply->fwdnum = bits_to_value(ptr, 24, 8);
  path_reply->prange_len = path_reply->uni_dst ? (bits_to_value(ptr, 32, 1) ? 24 : 16) : 0;
  path_reply->prange = path_reply->prange_len ? bits_to_value(ptr, 32, path_reply->prange_len) : 0;
  path_reply->drange_len = (path_reply->uni_dst && path_reply->obo_dep) ? (bits_to_value(ptr, 32 + path_reply->prange_len, 1) ? 24 : 16) : 0;
  path_reply->drange = path_reply->drange_len ? bits_to_value(ptr, 32 + path_reply->prange_len, path_reply->drange_len) : 0;
}

void sl_mesh_path_confirmation_from_buf(const uint8_t *ptr, path_confirmation_t *path_confirmation)
{
  path_confirmation->path_origin = bits_to_value(ptr, 0, 16);
  path_confirmation->path_target = bits_to_value(ptr, 16, 16);
}

void sl_mesh_dependent_node_update_from_buf(const uint8_t *ptr, dependent_node_update_t *dependent_node_update)
{
  dependent_node_update->type = bits_to_value(ptr, 0, 1);
  dependent_node_update->prohib = bits_to_value(ptr, 1, 7);
  dependent_node_update->path_endpoint = bits_to_value(ptr, 8, 16);
  dependent_node_update->drange_len = bits_to_value(ptr, 24, 1) ? 24 : 16;
  dependent_node_update->drange = bits_to_value(ptr, 24, dependent_node_update->drange_len);
}

void sl_mesh_path_request_solicitation_from_buf(const uint8_t *ptr, path_request_solicitation_t *path_request_solicitation)
{
  path_request_solicitation->addr = (uint16_t*)ptr;
}
                                      ...
                                      trans_pdu_t trans_pdu)
{
  ...
  size_t pdu_len = ((net_pdu.ctl ? MAX_TRANS_CONTROL_PDU_LEN : MAX_TRANS_ACCESS_PDU_LEN) - 4) * (trans_pdu.seg_n + 1);
  ...
  r->src = net_pdu.src;
  r->dst = net_pdu.dst;
  r->seq_0 = trans_pdu.seq_0;
  r->seg_n = trans_pdu.seg_n;
  r->ctl = net_pdu.ctl;
  r->ctl_opcode = trans_pdu.opcode;
  r->akf = trans_pdu.akf;
  r->aid = trans_pdu.aid;
  r->szmic = trans_pdu.szmic;
  r->bitmap = 0x00000000;
  r->pdu_len = pdu_len;
  ...
  if (net_pdu.seq < 0x2000) {
    r->seq_auth = trans_pdu.seq_0;
  } else {
    r->seq_auth = (net_pdu.seq & 0xffffe000) | trans_pdu.seq_0;
    if (r->seq_auth > net_pdu.seq) {
      r->seq_auth -= 8192;
    }
  }
...
sl_status_t sl_mesh_reassembler_add_segment(struct sl_mesh_decoder *context,
                                            ...
                                            bool *now_completed)
{
  if (!context || !valid_net_key(context, net_key) || !net_pdu.src || net_pdu.src > 0x7fff || !net_pdu.dst || trans_pdu.seg_o > trans_pdu.seg_n || !trans_pdu.segment || !trans_pdu.segment_len || !now_completed) {
    return SL_STATUS_INVALID_PARAMETER;
  }

  // All but last segment must be full length
  size_t expected_len = (net_pdu.ctl ? MAX_TRANS_CONTROL_PDU_LEN : MAX_TRANS_ACCESS_PDU_LEN) - 4;
  if (trans_pdu.seg_o < trans_pdu.seg_n) {
    if (trans_pdu.segment_len != expected_len) {
      return SL_STATUS_INVALID_PARAMETER;
    }
  }

  struct reassembler *r = find_reassembler(context, net_pdu.src, net_pdu.dst, trans_pdu.seq_0);
  if (!r) {
    sl_status_t sc = create_reassembler(context, net_key, &r, net_pdu, trans_pdu);
    if (sc) {
      return sc;
    }
  }
  ...
  if (net_pdu.ctl != r->ctl || net_key != r->net_key || trans_pdu.seg_n != r->seg_n) {
    return SL_STATUS_INVALID_PARAMETER;
  }

  if (r->ctl) {
    if (trans_pdu.opcode != r->ctl_opcode) {
      return SL_STATUS_INVALID_PARAMETER;
    }
  } else {
    if (trans_pdu.akf != r->akf || trans_pdu.aid != r->aid || trans_pdu.szmic != r->szmic) {
      return SL_STATUS_INVALID_PARAMETER;
    }
  }
  ...
  *now_completed = false;
  ...
  size_t pdu_off = trans_pdu.seg_o * expected_len;
  memcpy(r->pdu + pdu_off, trans_pdu.segment, trans_pdu.segment_len);
  r->bitmap |= (1 << trans_pdu.seg_o);

  // Last segment may need trimming
  if (trans_pdu.seg_o == r->seg_n) {
    r->pdu_len -= (expected_len - trans_pdu.segment_len);
  }
                                               ...
                                               trans_pdu_t trans_pdu)
{
  ...
  uint8_t nonce[13];
  size_t k, cipher_len;
  size_t mic_len = trans_pdu.szmic ? 8 : 4;
  bool virtual_addr;
  sl_status_t sc;

  if (!context || !trans_pdu.segment || !pdu || !matching_app_key || !matching_label_uuid) {
    return SL_STATUS_INVALID_PARAMETER;
  }

  if (!net_pdu.src || net_pdu.src > 0x7fff || !net_pdu.dst) {
    return SL_STATUS_INVALID_PARAMETER;
  }

  if (net_pdu.dst > 0x7fff && !trans_pdu.akf) {
    return SL_STATUS_INVALID_PARAMETER;
  }

  if (trans_pdu.segment_len > SL_MESH_MAX_ACCESS_PDU_LEN || trans_pdu.segment_len < 1 + mic_len) {
    return SL_STATUS_INVALID_PARAMETER;
  }

  virtual_addr = ((net_pdu.dst & 0xc000) == 0x8000) ? true : false;

  cipher_len = trans_pdu.segment_len - mic_len;

  if (current_ivindex && (current_ivindex & 0x1) != net_pdu.ivi) {
    // If low bit does not match, use the previous IV index value
    current_ivindex--;
  }
  ...
  for (k = 0; k < MAX_KEYS; k++) {
    ...
    if (trans_pdu.akf) {
      ...
      if (trans_pdu.aid != context->key[k].app.aid) {
        continue;
      }
    } else {
      ...
      if (net_pdu.src != context->key[k].dev.addr && net_pdu.dst != context->key[k].dev.addr) {
        continue;
      }
    }

    nonce[0] = trans_pdu.akf ? 0x01 : 0x02; // Nonce type
    nonce[1] = trans_pdu.szmic ? 0x80 : 0x00; // ASZMIC and Pad
    value2mem(net_pdu.seq, nonce + 2, 3); // SEQ
    value2mem(net_pdu.src, nonce + 5, 2); // SRC
    value2mem(net_pdu.dst, nonce + 7, 2); // DST
    value2mem(current_ivindex, nonce + 9, 4); // IV index

    if (virtual_addr) { // Virtual address -- try matching labels
      size_t v;
      for (v = 0; v < context->virtual_label_count; v++) {
        if (context->virtual_label[v].hash == net_pdu.dst) {
      ...
      sc = aes_128_ccm_decrypt(trans_pdu.akf
