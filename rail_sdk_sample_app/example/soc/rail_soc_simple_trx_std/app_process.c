...
static void handle_transmit(sl_rail_handle_t rail_handle)
{
  ...
  if (status != SL_RAIL_STATUS_NO_ERROR) {
    app_log_warning("sl_rail_sdk_ieee802154_transmission() status: 0x%08lX\n", status);
  }
  ...
  if (status != packet_size) {
    app_log_warning("BLE sl_rail_write_tx_fifo status: 0x%08lX\n", status);
  }
  status = sl_rail_start_tx(rail_handle, BLE_CHANNEL, SL_RAIL_TX_OPTIONS_DEFAULT, NULL);
  if (status != SL_RAIL_STATUS_NO_ERROR) {
    app_log_warning("BLE sl_rail_start_tx status: 0x%08lX\n", status);
  }
  #else
#endif
}
...
static void printf_ble_packet(const sl_rail_sdk_ble_advertising_packet_t *packet)
{
  ...
  uint8_t *packet_byte;
  app_log_info("BLE Packet: (size = %lu) (payload_len = %lu) {\n", packet_size, payload_len);
  for (uint32_t i = 0; i < packet_size; ++i) {
    packet_byte = (uint8_t *)packet + i;
    app_log_info("[%lu] -> 0x%02X\n", i, *packet_byte);
  }
  app_log_info("}\n");
}
...
{
  uint8_t i = 0;
  app_log_info("BLE Packet has been received. Payload (%u): ", length);
  for (i = 0; i < length; i++) {
