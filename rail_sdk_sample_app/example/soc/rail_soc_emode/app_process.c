  ...
  switch (app_state) {
    ...
    case S_CW:
      ...
      if (rail_status != SL_RAIL_STATUS_NO_ERROR) {
        app_log_warning("sl_rail_start_tx_stream() result: 0x%08lX", rail_status);
      }
      app_state = S_IDLE;
      break;
    case S_RX:
      app_log_info("Rx mode; EM%u\n", sleep_mode);
      set_radio_to_idle_state(rail_handle);
      rail_status = sl_rail_start_rx(rail_handle, get_selected_channel(), NULL);
      if (rail_status != SL_RAIL_STATUS_NO_ERROR) {
        app_log_warning("sl_rail_start_rx() result: 0x%08lX", rail_status);
      }
      app_state = S_IDLE;
      break;
    case S_PERIODIC_RX:
    ...
    case S_SET_POWER_LEVEL:
      ...
      if (is_raw) {
        ...
        if (rail_status != SL_RAIL_STATUS_NO_ERROR) {
          app_log_warning("sli_rail_set_tx_power() result: 0x%08lX", rail_status);
        }
      } else {
        rail_status = sl_rail_set_tx_power_dbm(rail_handle, power_deci_dbm);
        power_raw = sli_rail_get_tx_power(rail_handle);
        if (rail_status != SL_RAIL_STATUS_NO_ERROR) {
          app_log_warning("sl_rail_set_tx_power_dbm() result: 0x%08lX", rail_status);
        }
      }
    ...
    case S_RX_ERROR:
      ...
      if (periodic_receive) {
        ...
        if (rail_status != SL_RAIL_STATUS_NO_ERROR) {
          app_log_warning("sl_rail_start_scheduled_rx() result: 0x%08lX", rail_status);
        }
...
static void handle_periodic_rx(sl_rail_handle_t rail_handle)
{
  ...
  if (init_needed) {
    ...
    app_log_info(
      "Periodic Rx mode, sleepPeriod=%lu, rxPeriod=%lu; EM%u (sleep); EM%u (active)\n",
...
static void handle_periodic_tx(sl_rail_handle_t rail_handle)
{
  ...
  if (init_needed) {
    ...
    app_log_info("Periodic Tx mode, period=%lu; EM%u (sleep); EM%u (active)\n",
  ...
  if (!packet_sending) {
    ...
    if (rail_status != SL_RAIL_STATUS_NO_ERROR) {
      app_log_warning("sl_rail_start_scheduled_tx() result: 0x%08lX", rail_status);
    }
    packet_sending = true;
  }
}
