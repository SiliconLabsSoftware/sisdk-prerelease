...
static struct reg *find_free(void)
{
  ...
  if (registry.index == registry.length) {
    ...
    if (sl_memory_realloc(registry.entries,
                          (registry.length + registry.increment) * sizeof(struct reg),
...
sl_status_t mesh_lib_init(size_t initial_num, size_t increment)
{
  ...
  if (registry.length > 0) {
    if (sl_memory_calloc(registry.length,
                         sizeof(struct reg),
...
void mesh_lib_deinit(void)
{
  if (registry.entries != NULL) {
    sl_memory_free(registry.entries);
    registry.entries = NULL;
    registry.length = 0;
    registry.index = 0;
