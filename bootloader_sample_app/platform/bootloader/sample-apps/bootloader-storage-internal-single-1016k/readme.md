# Internal Storage Bootloader (Single Image on 1016 KB Device)

This sample configuration of the Gecko bootloader configures the bootloader to use the internal main flash to store firmware update images. The storage configuration is set up to store a single firmware update image at a time, in a single storage slot. The storage slot matches the 1016 kB reference layout: Slot0 at **0x08078000**–**0x080F1FFF** (offset **0x78000** from **0x08000000**), size **488 kB** (**0x7A000**, 499712 bytes), between the main application region and NVM3. This can be configured on the *"Bootloader Storage Slot Setup"*.
