...
iostream_eusart = OverrideUtil(euart_signals_map, 'eusart')


def compatible(provides: Set[str], board: Hardware) -> List[str]:
  return iostream_eusart.compatible(board)


def configure(project: Project_Config, board: Hardware, instance_name: str):
  config_prefix = f'SL_IOSTREAM_EUSART_{instance_name.upper()}'
  locations = iostream_eusart.configure(project, instance_name, config_prefix)

  is_special_case = any(board.provides(brd) for brd in special_case)

  flow_control_type = 'SL_IOSTREAM_EUSART_UART_FLOW_CTRL_NONE'
  if board.provides('device_has_radio') and not board.provides('device_supports_zwave') and not is_special_case:
    ...
    if 'cts_n_clk' in locations.keys() and 'rts_n_cs_n' in locations.keys():
      flow_control_type = 'SL_IOSTREAM_EUSART_UART_FLOW_CTRL_CTS_RTS'
    elif 'cts_n_clk' in locations.keys():
      flow_control_type = 'SL_IOSTREAM_EUSART_UART_FLOW_CTRL_CTS'
    elif 'rts_n_cs_n' in locations.keys():
      flow_control_type = 'SL_IOSTREAM_EUSART_UART_FLOW_CTRL_RTS'
