/***************************************************************************//**
 * @file
 * @brief App code related to the Sleepy-to-Sleepy shades device.
 *******************************************************************************
 * # License
...
void sl_zigbee_af_stack_status_cb(sl_status_t status)
{
  ...
  // in a non-S2S network context where the node type is not an S2S target.
  sl_zigbee_node_type_t nodeType;
  sl_status_t ret = sl_zigbee_af_get_node_type(&nodeType);
  if ((ret == SL_STATUS_NOT_JOINED)
      || (ret == SL_STATUS_OK && nodeType != SL_ZIGBEE_S2S_TARGET_DEVICE)) {
    return;
  }
...
bool sl_zigbee_af_pre_command_received_cb(sl_zigbee_af_cluster_command_t* cmd)
{
  if (cmd->commandId == ZCL_ON_COMMAND_ID) {
    led_turn_on(LED0);
    led_turn_on(LED1);
  } else if (cmd->commandId == ZCL_OFF_COMMAND_ID) {
    led_turn_off(LED0);
    led_turn_off(LED1);
  } else if (cmd->commandId == ZCL_TOGGLE_COMMAND_ID) {
    led_toggle(LED0);
    led_toggle(LED1);
  }
  ...
  sl_zigbee_af_core_println(
    "SWITCH ZCL %sEUI [%02X %02X %02X %02X %02X %02X %02X %02X]",
    (cmd->commandId == ZCL_ON_COMMAND_ID) ? "ON "
    : ((cmd->commandId == ZCL_OFF_COMMAND_ID) ? "OFF "
       : (cmd->commandId == ZCL_TOGGLE_COMMAND_ID) ? "TOGGLE " : ""),
    SwitchEUI[7],
    SwitchEUI[6],
    SwitchEUI[5],
    SwitchEUI[4],
                                       ...
                                       sl_zigbee_join_decision_t decision)
{
  ...
  if (sl_zigbee_af_get_node_type(&nodeType) == SL_STATUS_OK) {
    if (nodeType != SL_ZIGBEE_S2S_TARGET_DEVICE) {
      return;
    }
  }
...
void sl_zigbee_sleepy_to_sleepy_set_target_wakeup_period(SL_CLI_COMMAND_ARG)
{
  uint16_t targetWakeUpPeriodMs;

  if (sl_cli_get_argument_count(arguments) > 0) {
    targetWakeUpPeriodMs = sl_cli_get_argument_uint16(arguments, 0);
    sli_mac_set_mac_csl_period(0, targetWakeUpPeriodMs);
  } else {
    targetWakeUpPeriodMs = sli_mac_get_mac_csl_period(0);
  }

  sl_zigbee_af_cli_println("Target Wake Up Period: %d ms", targetWakeUpPeriodMs);
}

void sl_zigbee_sleepy_to_sleepy_set_target_rx_sample_time(SL_CLI_COMMAND_ARG)
{
  uint16_t targetRxSampleTimeMs;

  if (sl_cli_get_argument_count(arguments) > 0) {
    targetRxSampleTimeMs = sl_cli_get_argument_uint16(arguments, 0);
    sli_mac_set_mac_csl_channel_sample(0, targetRxSampleTimeMs);
  } else {
    targetRxSampleTimeMs = sli_mac_get_mac_csl_channel_sample(0);
  }

  sl_zigbee_af_cli_println("Target Wake Up Period: %d ms", targetRxSampleTimeMs);
}
