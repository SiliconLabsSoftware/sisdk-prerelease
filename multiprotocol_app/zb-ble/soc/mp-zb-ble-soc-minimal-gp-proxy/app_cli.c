...
void hal_reboot(sl_cli_command_arg_t *arguments)
{
  (void) arguments;
  halReboot();
}
void form_network_command(sl_cli_command_arg_t *arguments)
{
  ...
  sl_zigbee_af_cli_println("Form status=0x%08X", status);
}
...
void join_network_command(sl_cli_command_arg_t *arguments)
{
  ...
  sl_zigbee_node_type_t nodeType = (sl_zigbee_node_type_t)SLI_ZIGBEE_PRIMARY_NETWORK_DEVICE_TYPE;
  ...
  sl_zigbee_af_cli_println("Join status=0x%08X", status);
}
...
void permit_join_command(sl_cli_command_arg_t *arguments)
{
  ...
  sl_zigbee_af_cli_println("Permit join set 0x%08X", status);
}
...
void set_security_command(sl_cli_command_arg_t *arguments)
{
  ...
  if (securityLevel > 0
    ...
    if (paramLength > 0) {
      if (paramLength != SL_ZIGBEE_ENCRYPTION_KEY_SIZE) {
        sl_zigbee_af_cli_println("wrong param");
        return;
      }
  ...
  sl_zigbee_af_cli_println("Security set 0x%08X", status);
}
...
{
  sl_zigbee_af_cli_println("node [(>)%02X%02X%02X%02X%02X%02X%02X%02X]",
                           eui64[7],
                           eui64[6],
                           eui64[5],
                           eui64[4],
...
void info_command(sl_cli_command_arg_t *arguments)
{
  ...
  sl_zigbee_af_cli_println("panID [0x%04X] nodeID [0x%04X]",
                           networkParams.panId,
                           sl_zigbee_get_node_id());
  sl_zigbee_af_cli_println("parentID [0x%04X] parentRssi [%d]",
                           sl_zigbee_get_parent_id(),
                           sl_zigbee_get_avg_parent_rssi());
  sl_zigbee_af_cli_println("nodeType [%d], securityLevel [%d], networkState [%02X]",
                           nodeType,
...
void keys_print_command(sl_cli_command_arg_t *arguments)
{
  ...
  sl_zigbee_af_cli_println("NWK Key out FC: %04x", network_key_info.network_key_frame_counter);
  sl_zigbee_af_cli_println("NWK Key seq num: 0x%02X", network_key_info.network_key_sequence_number);
  sl_zigbee_af_cli_print("NWK Key: ");
  ...
  sl_zigbee_af_cli_println("TC Link Key out FC: %04x", key_data.outgoing_frame_counter);
  sl_zigbee_af_cli_print("TC Link Key: ");
  print_key_data(plaintext_key.key);
}
