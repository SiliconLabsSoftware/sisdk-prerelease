...
void sl_wisun_coap_rhnd_init(void)
{
  ...
  EFM_ASSERT(_wisun_coap_resource_mtx != NULL);
...
__STATIC_INLINE void _coap_resource_mutex_check_acquire(void)
{
  if (osKernelGetState() == osKernelRunning) {
    EFM_ASSERT(osMutexAcquire(_wisun_coap_resource_mtx, osWaitForever) == osOK);
  }
}
...
{
  if (osKernelGetState() == osKernelRunning) {
    EFM_ASSERT(osMutexRelease(_wisun_coap_resource_mtx) == osOK);
  }
}
...
#define __cleanup_service()                 \
  ...
  SL_COAP_SERVICE_LOOP() {
    ...
    EFM_ASSERT(sockid != SOCKET_INVALID_ID);
    ...
    EFM_ASSERT(sockid_rd != SOCKET_INVALID_ID);
