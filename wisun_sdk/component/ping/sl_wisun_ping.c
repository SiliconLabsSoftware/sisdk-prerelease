                          ...
                          sl_wisun_ping_req_resp_done_hnd_t req_resp_sent_hnd)
{
  ...
  for (uint16_t i = 0U; i < packet_count; ++i) {
    ...
    // pop up messages
    for (uint32_t j = 0UL; j < resp_cnt; ++j) {
  ...
  packet_ok_count = (uint16_t)(sum_resp_pkt - stat->lost);
  if (packet_ok_count) {
