...
sl_status_t app_settings_set(char *const domain_and_key, const char *const value_str)
{
  ...
  while (iter && iter->key) {
    if (!sl_strcasecmp(domain, app_settings_domain_str[iter->domain])
        && !sl_strcasecmp(iter->key, key)) {
      if (iter->set_handler) {
...
sl_status_t app_settings_get(char *const domain_and_key)
{
  ...
  iter = app_settings_entries;
  while (iter && iter->key) {
    if ((!domain || !sl_strcasecmp(domain, app_settings_domain_str[iter->domain]))
        && (!key || !sl_strcasecmp(iter->key, key))
        && iter->get_handler
...
sl_status_t app_settings_help(char *const domain_and_key, bool get)
{
  ...
  while (iter && iter->key) {
    bool domain_match = (!domain || !sl_strcasecmp(domain, app_settings_domain_str[iter->domain]));
    bool key_match = (!key || !sl_strcasecmp(iter->key, key));
                                    ...
                                    const app_settings_entry_t *entry)
{
  ...
  sl_strcpy_s(entry_value_str, entry->value_size - 1, value_str);
                                        ...
                                        const app_settings_entry_t *entry)
{
  ...
  if (sl_strcasecmp("ip_address_global", entry->key) == 0) {
    address_type = SL_WISUN_IP_ADDRESS_TYPE_GLOBAL;
  } else if (sl_strcasecmp("ip_address_link_local", entry->key) == 0) {
    address_type = SL_WISUN_IP_ADDRESS_TYPE_LINK_LOCAL;
  } else if (sl_strcasecmp("ip_address_border_router", entry->key) == 0) {
    address_type = SL_WISUN_IP_ADDRESS_TYPE_BORDER_ROUTER;
  } else if (sl_strcasecmp("ip_address_primary_parent", entry->key) == 0) {
    address_type = SL_WISUN_IP_ADDRESS_TYPE_PRIMARY_PARENT;
  } else if (sl_strcasecmp("ip_address_secondary_parent", entry->key) == 0) {
...
static void app_help_print_and_pad(const char *permission, const app_settings_entry_t *entry)
{
  ...
  size_t string_length = sl_strlen(permission) + sl_strlen(app_settings_domain_str[entry->domain]) + sl_strlen(entry->key);
