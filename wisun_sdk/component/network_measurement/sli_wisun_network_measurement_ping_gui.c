...
static void _set_packet_count(void *args)
{
  ...
  _meas_settings.packet_count = (uint16_t)val->value;
...
{
  const app_enum_t *val = (const app_enum_t *) args;
  _meas_settings.packet_length = (uint16_t)val->value;
  sl_display_renderer(_settings_form, NULL, 0);
}
...
__STATIC_INLINE uint8_t _calc_packet_loss_percent(const sl_wisun_nwm_node_stat_t *stat)
{
  return stat->ping_stat.packet_count ? (uint8_t)((stat->ping_stat.lost * 100U) / stat->ping_stat.packet_count) : 0xFFU;
}

static void _set_test_result_txtbox(void *args)
{
  const sl_wisun_nwm_node_stat_t *stat = (sl_wisun_nwm_node_stat_t *) args;
