...
/// CS Manager connection parameters
typedef struct {
...
typedef struct {
  ...
  uint8_t conn_phy; ///< Requested connection PHY
  uint8_t actual_conn_phy; ///< Actual connection PHY
  uint8_t request_conn_phy; ///< 1 to request connection PHY update
  int8_t max_tx_power_dbm; ///< Maximum TX power in dBm
  bool is_central; ///< true if the device acts as central, false for peripheral
  bool is_initiator; ///< true if the device acts as initiator, false for reflector
...
/// Negotiated procedure parameters
typedef struct {
  uint32_t subevent_len; ///< Subevent length
  uint16_t subevent_interval; ///< Subevent interval
  uint16_t event_interval; ///< Event interval
  uint16_t procedure_interval; ///< Procedure interval
  uint16_t procedure_count; ///< Procedure count
  uint16_t max_procedure_len; ///< Maximum procedure length
  uint8_t  antenna_config; ///< Antenna configuration index
  int8_t   tx_power; ///< Actual TX power
  uint8_t  subevents_per_event; ///< Number of subevents per event
} cs_procedure_parameter_info_t;
...
/**************************************************************************//**
 * Retrieve the negotiated procedure parameter information.
 *
 * @note It is to be called from CS Manager event handler for
 *       CS_MANAGER_EVENT_PROCEDURE_START_COMPLETE event.
 *
 * @param[in]  conn_handle Connection handle.
 * @param[out] info  Pointer to the parameters to populate.
 * @return Status of the operation.
 * @retval SL_STATUS_OK            Parameters retrieved successfully.
 * @retval SL_STATUS_NULL_POINTER  @p params_out is NULL.
 * @retval SL_STATUS_INVALID_STATE Procedure is not enabled.
 * @retval SL_STATUS_NOT_FOUND     No matching instance found for the
 *                                 given @p conn_handle.
 *****************************************************************************/
 ...
 *       are required; passing NULL members to clear them is not supported.
 *
 ...
 * @return Status of the operation.
 * @retval SL_STATUS_OK               Parameters populated successfully.
 * @retval SL_STATUS_NULL_POINTER     @p instance_config is NULL.
 *****************************************************************************/
...
/**************************************************************************//**
 * Retrieve default procedure parameters.
 *
...
/**************************************************************************//**
 * Set the default connection parameters used for new connections.
 *
 * Stores @p params as the active default connection parameters and applies them
 * via @ref sl_bt_connection_set_default_parameters so subsequent connections
 * open with these values. The stored values also become the baseline that
 * @ref cs_manager_create compares against, so a connection opened with matching
 * parameters skips the per-connection @ref sl_bt_connection_set_parameters
 * update.
 *
 * @note Must be called after the Bluetooth stack has booted.
 *
 * @param[in] params Pointer to the connection parameters to apply.
 * @return Status of the operation.
 * @retval SL_STATUS_OK            Parameters stored and applied successfully.
 * @retval SL_STATUS_NULL_POINTER  @p params is NULL.
 *****************************************************************************/
