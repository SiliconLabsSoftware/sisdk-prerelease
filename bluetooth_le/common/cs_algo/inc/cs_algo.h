...
SL_ENUM(cs_algo_error_t) {
  ...
  CS_ALGO_ERROR_CREATE_FAILED = 0,                    ///< Estimator/instance creation failed
  CS_ALGO_ERROR_CONFIGURE_FAILED,                     ///< Algo configuration failed
  CS_ALGO_ERROR_REMOVE_FAILED,                        ///< Instance removal failed
  CS_ALGO_ERROR_INVALID_RAS_DATA,                     ///< Invalid/unprocessable RAS data passed
  CS_ALGO_ERROR_RESULT_APPEND_FAILED,                 ///< Appending a result to the buffer failed
  CS_ALGO_ERROR_RUNTIME_FAILED,                       ///< Runtime context/operation failed
...
typedef struct {
  uint32_t data_size;                                  ///< Number of valid bytes of data
  const uint8_t *data;                                 ///< RAS ranging data body (caller-owned, read-only)
} cs_algo_ranging_buffer_t;

/// Ranging result for a CS procedure
typedef struct {
  uint8_t num_steps;                             ///< Number of steps in the procedure
  const uint8_t *step_channels;                  ///< Channel used for each step (caller-owned, read-only)
  cs_algo_ranging_buffer_t initiator;            ///< Initiator RAS ranging data
  cs_algo_ranging_buffer_t reflector;            ///< Reflector RAS ranging data
} cs_algo_result_t;
...
/***************************************************************************//**
 * @brief Callback function types for reporting results.
 *
...
/***************************************************************************//**
 * @brief Callback function type for reporting intermediate results.
 *
...
typedef void (*cs_algo_on_error_t)(uint8_t conn_handle,
                                   ...
                                   sl_status_t sc);
/// Callback structure for results
typedef struct {
  ...
  cs_algo_on_error_t on_error; ///< Callback function for reporting errors.
} cs_algo_event_callback_t;

/***************************************************************************//**
 * @brief Callback function type for reporting process finished.
 *
 * @param[in] conn_handle connection handle.
 * @param[in] ranging_counter ranging counter.
 ...
 *
 * @param[in] cb Callback structure (@ref cs_algo_event_callback_t).
 *
