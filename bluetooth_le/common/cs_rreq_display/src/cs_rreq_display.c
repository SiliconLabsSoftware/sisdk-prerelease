...
extern cs_rreq_display_instance_t lcd_instance[CS_RREQ_CONFIG_MAX_CONNECTIONS];

static sl_status_t cs_rreq_display_measurement_modes(sl_bt_cs_mode_t mode,
                                                     uint8_t algo_mode);
static sl_status_t cs_rreq_display_get_instance(uint8_t conn_handle,
                                                uint8_t *instance_num);

static void cs_rreq_display_get_status_text(char* text,
                                            cs_rreq_display_status_t status);

// -----------------------------------------------------------------------------
// Public function definitions

/******************************************************************************
 * CS RREQ display show state.
 *****************************************************************************/
void cs_rreq_display_start_scanning()
{
  for (uint8_t i = 0; i < CS_RREQ_CONFIG_MAX_CONNECTIONS; i++) {
    if (lcd_instance[i].status == CS_RREQ_DISPLAY_STATUS_UNINITIALIZED) {
      lcd_instance[i].status = CS_RREQ_DISPLAY_STATUS_SCANNING;
      cs_rreq_display_write_text_to_instance(CS_RREQ_DISPLAY_STATE_SCANNING_TEXT,
                                             ROW_STATUS_VALUE,
                                             i);
    }
  }
...
/******************************************************************************
 * CS RREQ display clear every row of specified instance.
 *****************************************************************************/
sl_status_t cs_rreq_display_create_instance(uint8_t conn_handle)
{
  for (int i = 0; i < CS_RREQ_CONFIG_MAX_CONNECTIONS; i++) {
    if (lcd_instance[i].connection_id == SL_BT_INVALID_CONNECTION_HANDLE) {
      lcd_instance[i].connection_id = conn_handle;
      lcd_instance[i].status = CS_RREQ_DISPLAY_STATUS_UNINITIALIZED;
      memset(&lcd_instance[i].content, 0, sizeof(cs_rreq_display_content_t));
...
/******************************************************************************
 * CS RREQ display update display data.
 *****************************************************************************/
...
{
  uint8_t instance_num;
  sl_status_t status = cs_rreq_display_get_instance(conn_handle, &instance_num);
  if (status != SL_STATUS_OK) {
    return status;
  }
  cs_rreq_display_clear_instance(instance_num);
  cs_rreq_display_write_text_to_instance(CS_RREQ_DISPLAY_STATE_SCANNING_TEXT,
                                         ROW_STATUS_VALUE,
                                         instance_num);
  memset(&lcd_instance[instance_num].content, 0, sizeof(cs_rreq_display_content_t));
...
/******************************************************************************
 * CS RREQ display write text to a specified row.
 *****************************************************************************/
void cs_rreq_display_update_instance(uint8_t instance_num)
{
  char text[6] = "\0";
  cs_rreq_display_get_status_text(text, lcd_instance[instance_num].status);
  cs_rreq_display_write_text_to_instance(text, ROW_STATUS_VALUE, instance_num);
  // do not display numbers if the instance is not in connected state
  if (lcd_instance[instance_num].status != CS_RREQ_DISPLAY_STATUS_CONNECTED) {
    return;
  }
  ...
  cs_rreq_display_write_float_to_instance(lcd_instance[instance_num].content.likeliness, ROW_LIKELINESS_VALUE, instance_num);
  cs_rreq_display_write_float_to_instance(lcd_instance[instance_num].content.rssi_distance, ROW_RSSI_DISTANCE_VALUE, instance_num);
  // BER is not supported in PBR mode
  if (lcd_instance[instance_num].content.mode != sl_bt_cs_mode_pbr) {
    cs_rreq_display_write_float_to_instance(lcd_instance[instance_num].content.bit_error_rate, ROW_BIT_ERROR_RATE_VALUE, instance_num);
  }
}
                                          ...
                                          uint8_t algo_mode)
{
  ...
  sc = cs_rreq_display_measurement_modes(mode, algo_mode);
  if (sc != SL_STATUS_OK) {
    display_log_error("Error during showing measurement mode and "
                      "algo mode on LCD! [E: 0x%lx]" NL, sc);
  }
}
...
{
  switch (status) {
    case CS_RREQ_DISPLAY_STATUS_CONNECTED:
      strcpy(text, CS_RREQ_DISPLAY_STATE_CONNECTED_TEXT);
      break;
    case CS_RREQ_DISPLAY_STATUS_SCANNING:
      strcpy(text, CS_RREQ_DISPLAY_STATE_SCANNING_TEXT);
      break;
    case CS_RREQ_DISPLAY_STATUS_ESTIMATE:
      strcpy(text, CS_RREQ_DISPLAY_STATE_ESTIMATE_TEXT);
      break;
    case CS_RREQ_DISPLAY_STATUS_INITIALIZED:
      strcpy(text, CS_RREQ_DISPLAY_STATE_INITIALIZED_TEXT);
      break;
    case CS_RREQ_DISPLAY_STATUS_SPOOFED:
      strcpy(text, CS_RREQ_DISPLAY_STATE_SPOOFED_TEXT);
      break;
    case CS_RREQ_DISPLAY_STATUS_UNINITIALIZED:
      strcpy(text, CS_RREQ_DISPLAY_STATE_UNINITIALIZED_TEXT);
      break;
    default:
      strcpy(text, CS_RREQ_DISPLAY_STATE_UNINITIALIZED_TEXT);
      break;
...
/******************************************************************************
 * CS RREQ display measurement mode and object tracking mode.
 *****************************************************************************/
static sl_status_t cs_rreq_display_measurement_modes(sl_bt_cs_mode_t mode,
                                                     uint8_t algo_mode)
{
  ...
  if (strncat(string, CS_RREQ_DISPLAY_MODE_TEXT, (sizeof(string) - strlen(string) - 1u)) == NULL) {
    display_log_error("Failed to concat \'%s\' string!" NL,
                      CS_RREQ_DISPLAY_MODE_TEXT);
    sc = SL_STATUS_FAIL;
  }

  if (mode == sl_bt_cs_mode_rtt) {
    if (strncat(string, CS_RREQ_DISPLAY_MODE_RTT_TEXT, (sizeof(string) - strlen(string) - 1u)) == NULL) {
      display_log_error("Failed to concat \'%s\' string!" NL,
                        CS_RREQ_DISPLAY_MODE_RTT_TEXT);
      sc = SL_STATUS_FAIL;
    }
  } else {
    if (strncat(string, CS_RREQ_DISPLAY_MODE_PBR_TEXT, (sizeof(string) - strlen(string) - 1u)) == NULL) {
      display_log_error("Failed to concat \'%s\' string!" NL,
                        CS_RREQ_DISPLAY_MODE_PBR_TEXT);
      sc = SL_STATUS_FAIL;
    }
    cs_rreq_display_clear_row(ROW_BIT_ERROR_RATE_TEXT);
  ...
  if (algo_mode == SL_RTL_CS_ALGO_MODE_REAL_TIME_BASIC) {
    ...
    if (strncat(string, CS_RREQ_DISPLAY_AMODE_STATIONARY_OBJ_TEXT, (sizeof(string) - strlen(string) - 1u)) == NULL) {
      display_log_error("Failed to concat \'%s\' string!" NL,
                        CS_RREQ_DISPLAY_AMODE_STATIONARY_OBJ_TEXT);
      sc = SL_STATUS_FAIL;
    }
  }
  cs_rreq_display_write_text_to_instance(string, ROW_MODE, 0);
  return sc;
}
