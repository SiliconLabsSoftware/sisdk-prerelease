...
static void clear_instance_data(rreq_t *rreq, bool init_stage);
                           ...
                           cs_rreq_create_config_t *config)
{
  ...
  clear_instance_data(rreq, false);
...
void cs_rreq_init(void)
{
  // Reset RREQ slots
  for (uint8_t i = 0; i < CS_RREQ_CONFIG_MAX_CONNECTIONS; i++) {
    clear_instance_data(&rreq_instances[i], true);
  }
}
...
static void clear_instance_data(rreq_t *rreq, bool init_stage)
{
  ...
  reset_subevent_data(rreq, init_stage);
  ...
  if (init_stage) {
    rreq->state = RREQ_STATE_UNINITIALIZED;
  } else {
